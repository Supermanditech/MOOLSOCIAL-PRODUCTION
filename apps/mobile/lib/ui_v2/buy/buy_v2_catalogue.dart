...
// Recurring treatment for the approved public Store name.
class _StorefrontTextSheen extends StatefulWidget {
  const _StorefrontTextSheen({required this.stateKey, required this.child});

  final String stateKey;
  final Widget child;
  final double phase = 0;
  final Color baseColor = BuyV2Colors.ink;
  final Color highlightColor = BuyV2Colors.muted;
...
class _BuyV2OffersViewState extends State<BuyV2OffersView> {
  ...
  BuyV2OfferPublisherType? get _publisher =>
      widget.session.finiteOffersPublisher ??
      (_mool ? BuyV2OfferPublisherType.moolSocial : null);
  set _publisher(BuyV2OfferPublisherType? value) =>
      widget.session.finiteOffersPublisher = value;
          ...
          SliverToBoxAdapter(
            child: _OffersCategoryControl(
              ...
              onSourceChanged: (value) => setState(() {
                widget.session.featuredOffersMoolSocial = value;
                _publisher = value ? BuyV2OfferPublisherType.moolSocial : null;
              }),
              onFilter: () async {
                final choice = await _chooseOfferPublisher(context, _publisher);
                if (mounted && choice != null) {
                  setState(() {
                    _publisher = choice.publisher;
                    widget.session.featuredOffersMoolSocial =
                        choice.publisher == BuyV2OfferPublisherType.moolSocial;
                  });
                }
              },
              onTap: () async {
          ...
          if (products.isEmpty)
            ...
            SliverToBoxAdapter(
              child: BuyV2ProgressiveProductGrid(
                ...
                productCardBuilder: (product) => BuyV2ProductCard(
                  session: session,
                  product: product,
                  compact: true,
                  alignMediaAtTop: true,
  ...
  return showModalBottomSheet<String>(
    ...
    builder: (sheetContext) => _CatalogueCategorySheet(
      ...
      categories: [
        ...
        ..._offerCategories(session),
      ],
      selectedCategoryId: selected,
      title: 'Offer categories',
      keyPrefix: 'buy-offers-category',
      allLabel: 'All offers',
...
String _offerPublisherLabel(BuyV2OfferPublisherType? publisher) =>
    switch (publisher) {
      BuyV2OfferPublisherType.manufacturer => 'Manufacturers',
...
class _PublishedOfferPromotionState extends State<_PublishedOfferPromotion> {
  bool get _mool => widget.moolSocial;
  PageController? _controller;
  String _pageKey = '';
  List<({BuyV2PublishedOffer offer, BuyV2Product product})> get _entries =>
      widget.entries
          .where(
            (e) =>
                (e.offer.publisherType == BuyV2OfferPublisherType.moolSocial) ==
                _mool,
          )
          .toList();
  ...
  Widget build(BuildContext context) {
    ...
    final key = '$_mool:${entries.map((e) => e.offer.identity).join('|')}';
    if (_pageKey != key) {
      _controller?.dispose();
      final retained = entries.indexWhere(
        ...
        children: [
          if (entries.isEmpty)
            const Padding(
              ...
              child: Text('No current offers from this publisher.'),
            )
          else
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                return SizedBox(
                  ...
                  child: PageView.builder(
                    key: ValueKey('buy-offer-cinema-$_pageKey'),
                    controller: _controller,
                    itemCount: entries.length,
                    padEnds: false,
                    onPageChanged: (index) => setState(
                      () => widget.session.featuredOfferPublicationId =
                          entries[index].offer.identity,
                    ),
          ...
          if (entries.isNotEmpty)
            Padding(
              ...
              child: Text(
                '${selected < 0 ? 1 : selected + 1} / ${entries.length}',
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, sheetConstraints) {
        final shortViewport = sheetConstraints.maxHeight < 480;
        final minimumExtent = shortViewport ? .9 : .5;
        return Semantics(
          ...
          child: KeyedSubtree(
            ...
            child: DraggableScrollableSheet(
              ...
              initialChildSize: shortViewport ? 1 : .72,
              minChildSize: minimumExtent,
              shouldCloseOnMinExtent: !shortViewport,
              maxChildSize: 1,
              snap: true,
              snapSizes: shortViewport ? null : const [.72, 1],
              expand: false,
                ...
                padding: EdgeInsets.only(
                  bottom: MediaQuery.viewInsetsOf(context).bottom > 0
                      ...
                      : MediaQuery.viewPaddingOf(
                          context,
                        ).bottom.clamp(24.0, double.infinity),
                ),
                child: ClipRRect(
                                      ...
                                      child: LayoutBuilder(
                                        builder: (context, constraints) {
                                          ...
                                          return GridView.builder(
                                            ...
                                            key: ValueKey(
                                              widget.keyPrefix != null
                                                  ? '${widget.keyPrefix}-list'
                                            ...
                                            itemBuilder: (context, index) {
                                              ...
                                              return Semantics(
                                                ...
                                                label:
                                                    '${widget.title ?? '${session.destination.label} categories'}, '
                                                    '$displayLabel'
                                                    '${selected ? ', selected' : ''}',
                                                selected: selected,
                                                button: true,
                                                child: Material(
                                                  ...
                                                  child: InkWell(
                                                    key: ValueKey(
                                                      '${widget.keyPrefix ?? (widget.onSelected == null ? 'buy-category' : 'buy-store-category')}-${category.id}',
                                                    ),
                                                    onTap: () async {
                                                      HapticFeedback.selectionClick();
            ...
            for (final id in ['s-dog-food', 's-shampoo'])
              if (session.findProduct(id) case final product?)
                BuyV2FilterSheetAction(
                  ...
                  title: 'Review fixture: ${product.customerTitle}',
...
List<String> _publicStoreFulfilmentLabels(
  ...
  for (final product in products) {
    for (final option in session.deliveryOptionsFor(product)) {
      labels.add(switch (option) {
        BuyV2DeliveryOption.quick => 'Quick',
        BuyV2DeliveryOption.scheduled => 'Scheduled delivery',
        BuyV2DeliveryOption.courier => 'Courier delivery',
        BuyV2DeliveryOption.freight => 'Bulk delivery',
        BuyV2DeliveryOption.collection => 'Collect at store',
      });
    }
  }
      ...
      children: [
        ...
        if (products.isNotEmpty)
          BuyV2ProgressiveProductGrid(
            ...
            storageKey: _scope,
            semanticLabel:
                '${widget.product.customerSeller(widget.product.seller)} product catalogue preview',
            laneCount: 1,
            storeContext: true,
            onOpenProduct: widget.onOpenProduct,
          ),
...
class _PagedFullStoreCatalogueState extends State<_PagedFullStoreCatalogue> {
    ...
    if (widget.onOpenOtherStore != null) {
      ...
      final query = BuyV2CatalogueQuery(
        ...
        regionId:
            ...
            base.regionId,
        customerLocationKey: base.customerLocationKey,
  ...
  Widget build(BuildContext context) {
    ...
    final saved = widget.session
        ...
        .toList(growable: false);
    final moreStores = _searching ? null : _otherStores();
    ...
    return PopScope(
      ...
      child: Padding(
        ...
        child: BuyV2PagedProductCatalogue(
          ...
          scopeKey: _scope,
          storeContext: true,
          onResetFilters: () => setState(() => _maximumPrice = null),
        ...
        Expanded(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: context.buyTitle.copyWith(fontSize: 16)),
                const SizedBox(height: 2),
                Text(detail, style: context.buyMeta),
              ],
            ),
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            tooltip: 'Remove ${product.customerTitle} from Saved',
            onPressed: onRemove,
  ...
  Widget build(BuildContext context) {
    ...
    final seeProducts = Align(
      ...
      child: OutlinedButton.icon(
        ...
        style: BuyV2ActionStyle.button(
          OutlinedButton.styleFrom(
            minimumSize: const Size(44, BuyV2Metrics.minimumTap),
            side: const BorderSide(color: BuyV2Colors.line),
          ),
        ),
                          ...
                          SizedBox(
                            ...
                            child:
                                ...
                                BuyV2ProductCard(
                                  ...
                                  product: product,
                                  compact: true,
                                  alignMediaAtTop: alignMediaAtTop,
                                  savedContext: savedContext,
                                  storeContext: storeContext,
                                  onOpenProduct: onOpenProduct,
...
class _FeaturedProductCardState extends State<_FeaturedProductCard> {
  ...
  Widget _buildCard(BuildContext context, BoxConstraints constraints) {
    ...
    final fulfilmentMode = session.fulfilmentModeFor(product);
...
typedef _ProductGlanceField = ({String text, TextStyle style});

List<_ProductGlanceField> _productGlanceFields(
  BuyV2Session session,
  BuyV2Product product,
  bool storeContext,
) {
  ...
  return [
    ...
    if (!storeContext)
      (text: product.customerSeller(facts.partner), style: detail),
    if (!storeContext && product.sellerType.trim().isNotEmpty)
      (text: _sellerTypeLabel(product.sellerType), style: detail),
    (
      text: decision?.canAdd == false ? decision!.statusLabel : promise,
      style: detail.copyWith(
        color: decision?.canAdd == false
            ? BuyV2Colors.orange
            : BuyV2Colors.green,
        fontWeight: FontWeight.w700,
      ),
...
class _ProductGlance extends StatelessWidget {
  const _ProductGlance({
    required this.session,
    required this.product,
  ...
  Widget _buildAction(BuildContext context) {
    ...
    if (showRemoveLabel && saved) {
      return Semantics(
        key: ValueKey('buy-save-${product.id}'),
        button: true,
        label: 'Remove ${product.customerTitle} from Saved',
        child: Tooltip(
          message: 'Remove ${product.customerTitle} from Saved',
          child: InkWell(
            onTap: toggleSaved,
...
_compactProductVisualLayout(
  ...
  double minimumControlExtent = 44,
}) {
  var controlsBottom =
      minimumControlExtent; // Reserve the complete Save touch target.
  var badgeTop = 6.0;
...
class _ProductVisual extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      BuyV2CartAvoidanceRegion(child: LayoutBuilder(builder: _buildVisual));
