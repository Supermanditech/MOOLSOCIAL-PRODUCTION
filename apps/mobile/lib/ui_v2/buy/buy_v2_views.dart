...
import '../profile/global_profile_panel_v2.dart'
    show globalSecurityLocationForReturn;
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          product.composition ?? product.customerVariant,
                        ...
                        if (automaticFulfilment) ...[
                          ...
                          _ProductStoreSummary(
                            key: ValueKey(
                              'buy-product-hero-store-${product.id}',
                            ),
                            product: product,
                            facts: facts,
                            action: storeAction,
                            location:
                                trust.state == BuyV2MarketplaceTrustState.ready
                    ...
                    _DecisionPanel(
                      ...
                      children: [
                        _DecisionRow(
                          ...
                          label: 'Composition',
                          value: product.composition ?? product.customerVariant,
                        ),
                        _DecisionRow(
      ...
      children: [
        ...
        if (status != null)
          Text(
            ...
            style: context.buyMeta.copyWith(
              fontSize: 11,
              color: facts.storeOperatingState == BuyV2StoreOperatingState.open
                  ? BuyV2Colors.green
                  : BuyV2Colors.muted,
            ),
          ),
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          return Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              SizedBox(width: 82, child: labelText),
...
class _PublicProductOrderInformation extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final returns =
        _nonBlankComplianceValue(product.purchaseProtection?.summary) ??
        _nonBlankComplianceValue(product.returnPolicy);
    ...
    add('Delivery', buyerPromise, color: BuyV2Colors.green);
    final method = buyV2FulfilmentModeLabel(session.fulfilmentModeFor(product));
    add('Method', method);
    add(
      'Deliver to',
      session.selectedAddressOrNull?.shortLine ?? 'Choose a delivery address',
    );
    ...
    add('Returns', returns);
    if (normalize(product.returnPolicy ?? '') != normalize(returns ?? '')) {
      add('Store policy', product.returnPolicy);
    }
    return Container(
              ...
              for (final action in actions)
                SizedBox(
                  width:
                      ...
                      (accessibleActions ? 2 : actions.length),
...
class _WholesaleTradeActionDock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fulfilmentMode =
        facts.fulfilmentMode ?? BuyV2FulfilmentMode.standardCourier;
    ...
    if (!businessVerified) {
      ...
      actionGroup = _ProductPurchaseActionRow(
        ...
        addSemanticLabel:
            ...
            '${product.customerTitle} to Cart for ${buyV2Money(orderTotal)}. '
            '$deliveryDecision',
        onAdd: onAdd,
        onEdit: onEdit,
              ...
              builder: (context, constraints) {
                ...
                return Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 10),
                    SizedBox(width: actionWidth, child: actionGroup),
                  ],
                );
              },
            ),
...
class _ProductContinuationSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final railHeight = products.fold<double>(174, (height, item) {
              ...
              Semantics(
                ...
                child: SizedBox(
                  ...
                  child: ListView.separated(
                    key: ValueKey(
                      'buy-product-continuation-lane-${product.id}',
                    ),
  ...
  Widget build(BuildContext context) {
    ...
    return _DecisionPanel(
      ...
      children: [
        ...
        if (trust.returnSummary != product.purchaseProtection?.summary &&
    ...
    return Column(
      ...
      children: [
        ...
        if (!resolving && notice != null) ...[
          const SizedBox(height: 10),
          notice,
          if (session.collectionIdentity?.value == null)
            Align(
              alignment: Alignment.centerLeft,
              child: TextButton.icon(
                key: const ValueKey('buy-checkout-collection-sign-in'),
                onPressed: () => context.push(
                  globalSecurityLocationForReturn(
                    Uri(
                      ...
                      queryParameters: {
                        'view': 'checkout',
                        'sub': session.destination.name,
                        'scope': session.checkoutScope.name,
                      },
                    ).toString(),
                  ),
                ),
                icon: const Icon(Icons.login_rounded),
                label: const Text('Sign in options'),
              ),
            ),
        ],
        if (reviewing) ...[
      ...
      child: quantity > 0
          ...
          : ConstrainedBox(
              ...
              child: Semantics(
                label:
                    addSemanticLabel ??
                    'Add ${product.minimumOrder == 1 ? product.customerTitle : 'minimum order of ${product.minimumOrder} packs of ${product.customerTitle}'} to Cart',
                button: true,
                onTap: onAdd,
                excludeSemantics: true,
                child: OutlinedButton.icon(
