...
void main() {
  ...
  testWidgets('R51.13 five-stage Shop studio and context finales review', (
    tester,
  ) async {
    ...
    await tester.pump();
    await _capture(tester, 'r51-13-header-shop-studio-start');
    for (var stage = 1; stage <= 5; stage += 1) {
      await tester.pump(const Duration(milliseconds: 720));
      await _capture(tester, 'r51-13-header-shop-visual-stage-$stage');
    }
    await tester.pumpAndSettle();
    await _capture(tester, 'r51-13-header-shop-final-studio');
    ...
    for (final entry in const [
      ...
      await _capture(tester, 'r51-13-header-${entry.$2}-studio-mid');
      await tester.pump(const Duration(milliseconds: 1800));
      await tester.pumpAndSettle();
      await _capture(tester, 'r51-13-header-${entry.$2}-studio-final');
    }
  ...
  testWidgets('R51.13 compact and reduced-motion finales remain reviewable', (
    tester,
  ) async {
    tester.view.devicePixelRatio = 1;
    ...
    await tester.pumpAndSettle();
    await _capture(tester, 'r51-13-header-shop-320-140-final');
