...
void main() {
  ...
  test('runtime UI review isolates only the Google Services debug task', () {
    final gradle = File('android/app/build.gradle.kts').readAsStringSync();
    final start = gradle.indexOf('val runtimeReviewDefines =');
    final end = gradle.indexOf('val localPropertiesFile =', start);
    expect(start, greaterThanOrEqualTo(0));
    expect(end, greaterThan(start));
    final isolation = gradle.substring(start, end);
    expect(isolation, contains('Base64.getDecoder().decode(encoded)'));
    expect(isolation, contains('androidDebugPackage == "runtime"'));
    for (final flag in const [
      'MOOLSOCIAL_UI_REVIEW_ONLY=true',
      'MOOLSOCIAL_DEVICE_REVIEW=true',
      'MOOLSOCIAL_USE_EMULATORS=true',
    ]) {
      expect(isolation, contains('"$flag"'));
    }
    expect(isolation, contains('.all { it in runtimeReviewDefines }'));
    expect(isolation, contains('if (runtimeUiReview)'));
    expect(isolation, contains('it.name == "processDebugGoogleServices"'));
    expect(isolation, isNot(contains('processReleaseGoogleServices')));
    expect(isolation, isNot(contains('processProfileGoogleServices')));
  });
