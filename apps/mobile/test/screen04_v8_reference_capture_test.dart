...
void main() {
  ...
  testWidgets(
    'Screen 04 v8 identical-viewport comparison captures',
    (tester) async {
      ...
      await tester.pumpAndSettle();
      await _capture(tester, 'screen04-v8-videos-discovery-390x844.png');

      await tester.tap(find.text('5-minute morning mobility'));
      await tester.pumpAndSettle();
      await _capture(tester, 'screen04-v8-videos-watch-390x844.png');

      await tester.tap(find.byKey(const Key('screen04-video-details-trigger')));
      await tester.pumpAndSettle();
      await _captureOverlay(
        tester,
        'screen04-v8-videos-description-390x844.png',
      );
      ...
      await tester.pumpAndSettle();
      await _captureOverlay(tester, 'screen04-v8-videos-channel-390x844.png');
      ...
      await tester.pumpAndSettle();
      await _capture(tester, 'screen04-v8-shorts-390x844.png');

      await tester.tap(find.byKey(const Key('screen04-rail-feed')));
      await tester.pumpAndSettle();
      await _capture(tester, 'screen04-v8-feed-390x844.png');

      await tester.tap(find.byKey(const Key('screen04-rail-create')));
      await tester.pumpAndSettle();
      await _capture(tester, 'screen04-v8-create-390x844.png');
      expect(tester.takeException(), isNull);
    },
    // The immutable v8 HTML/reference package remains preserved. The later
    // founder-authorized YouTube review lane adds a first-class Shorts filter,
    // so the deployed-trial shell is locked by the separate operational
    // baseline capture.
    skip: true,
  );
}
