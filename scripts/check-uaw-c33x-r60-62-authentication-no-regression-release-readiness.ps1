...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33x.json',
...
function Assert-C33X {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33X r60.62 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33XFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33X -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33XSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33X -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33XPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33X -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33XManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33X -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33XFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33X -Condition (
...
$ticketId = 'UAW-C33X-R60-62-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33XFile `
  -Path 'config/uaw-c33x-r60-62-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33X ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33XSanitizedText -Text $ticketRaw -Label 'C33X ticket'
Assert-C33X -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '553514EBAD81981616EE4A61CA3FA10C60CEB4CD4AE40792B86AD23C4CAA5293'
...
Assert-C33X -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.62' -and
  [string]$ticket.candidate.versionCode -ceq '2026081362' -and
...
$uploadRunbookPath = Resolve-C33XFile `
  -Path 'docs/quality/UAW-C33X-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33X pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33XSanitizedText -Text $uploadRunbook -Label 'C33X upload runbook'
Assert-C33X -Condition (
...
$resolvedStatePath = Resolve-C33XFile -Path $StatePath -Label 'C33X state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33XSanitizedText -Text $stateRaw -Label 'C33X state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33XFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33X aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33XSanitizedText -Text $aggregateRaw -Label 'C33X aggregate'
...
Assert-C33X -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.62' -and
  [string]$state.candidate.versionCode -ceq '2026081362' -and
...
Assert-C33X -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33X-R60-62-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.62' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081362'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33XFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33X pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33XFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33X end-to-end founder authorization'
Assert-C33X -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33X-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33X-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33X-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33X -Condition (
  $historicalContracts.Count -eq 13 -and
  $aggregateHistoricalContracts.Count -eq 13
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33XFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33XFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33XFile `
...
$rejectedC33OStatePath = Resolve-C33XFile `
...
$rejectedC33PStatePath = Resolve-C33XFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33XFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33XFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C33XFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C33XFile `
...
$rejectedC33UStatePath = Resolve-C33XFile `
...
$rejectedC33VStatePath = Resolve-C33XFile `
...
$rejectedC33WStatePath = Resolve-C33XFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33XFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33XFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33XFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33XFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33X founder launcher'
$wrapperPath = Resolve-C33XFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33XFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33X interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C33XFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C33X predeclared source-cycle owner'
$resultRetentionPath = Resolve-C33XFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33XPowerShellOwner -Path $launcherPath -Label 'C33X founder launcher'
Assert-C33XPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33XPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33X interrupted-postbuild recovery owner'
Assert-C33XPowerShellOwner `
  -Path $cycleOwnerPath `
...
$gateNeedle = 'scripts/check-uaw-c33x-r60-62-authentication-no-regression-release-readiness.ps1'
...
Assert-C33X -Condition (
  ...
  $launcher.IndexOf('C33X r60.62 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33X founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33X binding, gate order or single appbundle owner changed.'
Assert-C33X -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33XFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33XFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33XFile `
...
$googleStatePath = Resolve-C33XFile `
...
Assert-C33XSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33XFile `
...
Assert-C33XSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C33XFile `
...
if ($cycles -eq 0) {
  ...
  Assert-C33XManifestCurrent -ManifestPath $manifestPath
  Assert-C33X -Condition (
    (Get-FileHash -Algorithm SHA256 -LiteralPath $focusedManifestPath).Hash -ceq
      [string]$state.sourceQualification.focusedManifestSha256
...
Write-Output (
  'C33X r60.62 no-regression release gate passed: ' +
