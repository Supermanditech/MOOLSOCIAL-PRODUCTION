...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33z.json',
...
function Assert-C33Z {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33Z r60.64 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33ZFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33Z -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33ZSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33Z -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33ZPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33Z -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33ZManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33Z -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33ZFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33Z -Condition (
...
$ticketId = 'UAW-C33Z-R60-64-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33ZFile `
  -Path 'config/uaw-c33z-r60-64-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33Z ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33ZSanitizedText -Text $ticketRaw -Label 'C33Z ticket'
Assert-C33Z -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '6C1437567B6973CBD2C5FC547DFDD015F43594CFB7FDDC4352C0037BEF09092A'
...
Assert-C33Z -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.64' -and
  [string]$ticket.candidate.versionCode -ceq '2026081364' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C33Y-R60-63-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  [string]$ticket.classificationReason -match 'REG2651' -and
...
$uploadRunbookPath = Resolve-C33ZFile `
  -Path 'docs/quality/UAW-C33Z-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33Z pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33ZSanitizedText -Text $uploadRunbook -Label 'C33Z upload runbook'
Assert-C33Z -Condition (
...
$resolvedStatePath = Resolve-C33ZFile -Path $StatePath -Label 'C33Z state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33ZSanitizedText -Text $stateRaw -Label 'C33Z state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33ZFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33Z aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33ZSanitizedText -Text $aggregateRaw -Label 'C33Z aggregate'
...
Assert-C33Z -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.64' -and
  [string]$state.candidate.versionCode -ceq '2026081364' -and
...
Assert-C33Z -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33Z-R60-64-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.64' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081364'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33ZFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33Z pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33ZFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33Z end-to-end founder authorization'
Assert-C33Z -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33Z-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33Z-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33Z-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33Z -Condition (
  $historicalContracts.Count -eq 15 -and
  $aggregateHistoricalContracts.Count -eq 15
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33ZFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33ZFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33ZFile `
...
$rejectedC33OStatePath = Resolve-C33ZFile `
...
$rejectedC33PStatePath = Resolve-C33ZFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33ZFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33ZFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C33ZFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C33ZFile `
...
$rejectedC33UStatePath = Resolve-C33ZFile `
...
$rejectedC33VStatePath = Resolve-C33ZFile `
...
$rejectedC33WStatePath = Resolve-C33ZFile `
...
$rejectedC33XStatePath = Resolve-C33ZFile `
...
$rejectedC33YStatePath = Resolve-C33ZFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33ZFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33ZFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33ZFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33ZFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33Z founder launcher'
$wrapperPath = Resolve-C33ZFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33ZFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33Z interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C33ZFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C33Z predeclared source-cycle owner'
$resultRetentionPath = Resolve-C33ZFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33ZPowerShellOwner -Path $launcherPath -Label 'C33Z founder launcher'
Assert-C33ZPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33ZPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33Z interrupted-postbuild recovery owner'
Assert-C33ZPowerShellOwner `
  -Path $cycleOwnerPath `
...
$gateNeedle = 'scripts/check-uaw-c33z-r60-64-authentication-no-regression-release-readiness.ps1'
...
Assert-C33Z -Condition (
  ...
  $launcher.IndexOf('C33Z r60.64 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33Z founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33Z binding, gate order or single appbundle owner changed.'
Assert-C33Z -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33ZFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33ZFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33ZFile `
...
$googleStatePath = Resolve-C33ZFile `
...
Assert-C33ZSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33ZFile `
...
Assert-C33ZSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C33ZFile `
...
Write-Output (
  'C33Z r60.64 no-regression release gate passed: ' +
