...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33q.json',
...
function Assert-C33Q {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33Q r60.55 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33QFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33Q -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33QSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33Q -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33QPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33Q -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33QManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33Q -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33QFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33Q -Condition (
...
$ticketId = 'UAW-C33Q-R60-55-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33QFile `
  -Path 'config/uaw-c33q-r60-55-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33Q ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33QSanitizedText -Text $ticketRaw -Label 'C33Q ticket'
Assert-C33Q -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '757AEFA05E3E96436324383D54F735D3E6BE39404074179AD7F8457ED961C7A4'
...
Assert-C33Q -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.55' -and
  [string]$ticket.candidate.versionCode -ceq '2026081355' -and
...
$uploadRunbookPath = Resolve-C33QFile `
  -Path 'docs/quality/UAW-C33Q-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33Q pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33QSanitizedText -Text $uploadRunbook -Label 'C33Q upload runbook'
Assert-C33Q -Condition (
...
$resolvedStatePath = Resolve-C33QFile -Path $StatePath -Label 'C33Q state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33QSanitizedText -Text $stateRaw -Label 'C33Q state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33QFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33Q aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33QSanitizedText -Text $aggregateRaw -Label 'C33Q aggregate'
...
Assert-C33Q -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.55' -and
  [string]$state.candidate.versionCode -ceq '2026081355' -and
...
Assert-C33Q -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33Q-R60-55-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.55' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081355'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33QFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33Q pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33QFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33Q end-to-end founder authorization'
Assert-C33Q -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33Q-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33Q-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33Q-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33Q -Condition (
  $historicalContracts.Count -eq 6 -and
  $aggregateHistoricalContracts.Count -eq 6
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33QFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33QFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33QFile `
...
$rejectedC33OStatePath = Resolve-C33QFile `
...
$rejectedC33PStatePath = Resolve-C33QFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33QFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33QFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33QFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33QFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33Q founder launcher'
$wrapperPath = Resolve-C33QFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33QFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33Q interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33QFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33QPowerShellOwner -Path $launcherPath -Label 'C33Q founder launcher'
Assert-C33QPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33QPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33Q interrupted-postbuild recovery owner'
Assert-C33QPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33q-r60-55-authentication-no-regression-release-readiness.ps1'
...
Assert-C33Q -Condition (
  ...
  $launcher.IndexOf(
    "throw 'C33Q founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33Q binding, gate order or single appbundle owner changed.'
Assert-C33Q -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33QFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33QFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33QFile `
...
$googleStatePath = Resolve-C33QFile `
...
Assert-C33QSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33QFile `
...
Assert-C33QSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33QManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33QFile `
...
Write-Output (
  'C33Q r60.55 no-regression release gate passed: ' +
