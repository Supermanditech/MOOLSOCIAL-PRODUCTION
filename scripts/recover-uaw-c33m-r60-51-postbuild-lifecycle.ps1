...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33m.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33M postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33MRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33M existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33MRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33MRecovery -Condition (
...
function Write-C33MRecoveryState {
  param(
  ...
  Assert-C33MRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$candidateId =
  'UAW-C33M-R60-51-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRoot =
  'artifacts/quality/uaw-c33m-r60-51-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33MRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33MRecoveryPath `
...
$transientGoogleServices = Resolve-C33MRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33MRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.51-2026081351-release.aab"
$sealedPath = Resolve-C33MRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33MRecoveryPath `
...
Assert-C33MRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33MRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.51' -and
  [string]$provenance.versionCode -ceq '2026081351' -and
...
$buildLog = Resolve-C33MRecoveryPath `
...
Assert-C33MRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33MRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33m-recovery-state-write'
  Write-C33MRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33m-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33m-r60-51-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33M existing-AAB postbuild lifecycle recovery passed: ' +
