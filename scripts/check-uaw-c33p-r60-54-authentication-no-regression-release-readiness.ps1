...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33p.json',
...
function Assert-C33P {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33P r60.54 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33PFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33P -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33PSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33P -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33PPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33P -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33PManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33P -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33PFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33P -Condition (
...
$ticketId = 'UAW-C33P-R60-54-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33PFile `
  -Path 'config/uaw-c33p-r60-54-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33P ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33PSanitizedText -Text $ticketRaw -Label 'C33P ticket'
Assert-C33P -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '7A2AD0661EFA6AD0E9993A91C1DAA8ABF145543292613EAB34F949067E24137A'
...
Assert-C33P -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.54' -and
  [string]$ticket.candidate.versionCode -ceq '2026081354' -and
...
$uploadRunbookPath = Resolve-C33PFile `
  -Path 'docs/quality/UAW-C33P-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33P pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33PSanitizedText -Text $uploadRunbook -Label 'C33P upload runbook'
Assert-C33P -Condition (
...
$resolvedStatePath = Resolve-C33PFile -Path $StatePath -Label 'C33P state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33PSanitizedText -Text $stateRaw -Label 'C33P state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33PFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33P aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33PSanitizedText -Text $aggregateRaw -Label 'C33P aggregate'
...
Assert-C33P -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.54' -and
  [string]$state.candidate.versionCode -ceq '2026081354' -and
...
Assert-C33P -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33P-R60-54-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.54' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081354'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33PFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33P pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33PFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33P end-to-end founder authorization'
Assert-C33P -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33P-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33P-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33P-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33P -Condition (
  $historicalContracts.Count -eq 5 -and
  $aggregateHistoricalContracts.Count -eq 5
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33PFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33PFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33PFile `
...
$rejectedC33OStatePath = Resolve-C33PFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33PFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33PFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33PFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33PFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33P founder launcher'
$wrapperPath = Resolve-C33PFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33PFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33P interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33PFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33PPowerShellOwner -Path $launcherPath -Label 'C33P founder launcher'
Assert-C33PPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33PPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33P interrupted-postbuild recovery owner'
Assert-C33PPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33p-r60-54-authentication-no-regression-release-readiness.ps1'
...
Assert-C33P -Condition (
  ...
  $launcher.IndexOf(
    "throw 'C33P founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33P binding, gate order or single appbundle owner changed.'
Assert-C33P -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33PFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33PFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33PFile `
...
$googleStatePath = Resolve-C33PFile `
...
Assert-C33PSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33PFile `
...
Assert-C33PSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33PManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33PFile `
...
Write-Output (
  'C33P r60.54 no-regression release gate passed: ' +
