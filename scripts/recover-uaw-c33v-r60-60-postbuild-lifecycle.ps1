...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33v.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33V postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33VRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33V existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33VRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33VRecovery -Condition (
...
function Write-C33VRecoveryState {
  param(
  ...
  Assert-C33VRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33v-r60-60-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33VRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33VRecoveryPath `
...
$transientGoogleServices = Resolve-C33VRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33VRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.60-2026081360-release.aab"
$sealedPath = Resolve-C33VRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33VRecoveryPath `
...
Assert-C33VRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33VRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.60' -and
  [string]$provenance.versionCode -ceq '2026081360' -and
...
$buildLog = Resolve-C33VRecoveryPath `
...
Assert-C33VRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33VRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33v-recovery-state-write'
  Write-C33VRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33v-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33v-r60-60-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33V existing-AAB postbuild lifecycle recovery passed: ' +
