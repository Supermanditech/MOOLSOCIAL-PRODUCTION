...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33u.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33U postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33URecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33U existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33URecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33URecovery -Condition (
...
function Write-C33URecoveryState {
  param(
  ...
  Assert-C33URecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33u-r60-59-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33URecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33URecoveryPath `
...
$transientGoogleServices = Resolve-C33URecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33URecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.59-2026081359-release.aab"
$sealedPath = Resolve-C33URecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33URecoveryPath `
...
Assert-C33URecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33URecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.59' -and
  [string]$provenance.versionCode -ceq '2026081359' -and
...
$buildLog = Resolve-C33URecoveryPath `
...
Assert-C33URecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33URecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33u-recovery-state-write'
  Write-C33URecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33u-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33u-r60-59-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33U existing-AAB postbuild lifecycle recovery passed: ' +
