...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33p.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33P postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33PRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33P existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33PRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33PRecovery -Condition (
...
function Write-C33PRecoveryState {
  param(
  ...
  Assert-C33PRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33p-r60-54-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33PRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33PRecoveryPath `
...
$transientGoogleServices = Resolve-C33PRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33PRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.54-2026081354-release.aab"
$sealedPath = Resolve-C33PRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33PRecoveryPath `
...
Assert-C33PRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33PRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.54' -and
  [string]$provenance.versionCode -ceq '2026081354' -and
...
$buildLog = Resolve-C33PRecoveryPath `
...
Assert-C33PRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33PRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33p-recovery-state-write'
  Write-C33PRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33p-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33p-r60-54-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33P existing-AAB postbuild lifecycle recovery passed: ' +
