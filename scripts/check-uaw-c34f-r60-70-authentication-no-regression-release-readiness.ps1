...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c34f.json',
...
function Assert-C34F {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34F r60.70 no-regression release gate rejected: $Message"
  }
}

function Resolve-C34FFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C34F -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C34FSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C34F -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C34FPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C34F -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C34FManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C34F -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C34FFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C34F -Condition (
...
$ticketId = 'UAW-C34F-R60-70-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C34FFile `
  -Path 'config/uaw-c34f-r60-70-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C34F ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C34FSanitizedText -Text $ticketRaw -Label 'C34F ticket'
Assert-C34F -Condition (
...
Assert-C34F -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.70' -and
  [string]$ticket.candidate.versionCode -ceq '2026081370' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C34E-R60-69-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  [string]$ticket.classificationReason -match 'REG1863' -and
  [string]$ticket.classificationReason -match 'REG2664' -and
  [string]$ticket.classificationReason -match 'REG2665' -and
  [string]$ticket.classificationReason -match 'REG2666' -and
  [string]$ticket.classificationReason -match 'REG2667' -and
...
$uploadRunbookPath = Resolve-C34FFile `
  -Path 'docs/quality/UAW-C34F-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260817.md' `
  -Label 'C34F pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C34FSanitizedText -Text $uploadRunbook -Label 'C34F upload runbook'
Assert-C34F -Condition (
...
$resolvedStatePath = Resolve-C34FFile -Path $StatePath -Label 'C34F state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C34FSanitizedText -Text $stateRaw -Label 'C34F state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C34FFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C34F aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C34FSanitizedText -Text $aggregateRaw -Label 'C34F aggregate'
...
Assert-C34F -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.70' -and
  [string]$state.candidate.versionCode -ceq '2026081370' -and
...
Assert-C34F -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34F-R60-70-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.70' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081370'
...
$founderAuthorizationPath = Resolve-C34FFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C34F end-to-end founder authorization'
Assert-C34F -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C34F-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260817.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C34F-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260817.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C34F-END-TO-END-FOUNDER-AUTHORIZATION-20260817.md' -and
...
Assert-C34F -Condition (
  $historicalContracts.Count -eq 21 -and
  $aggregateHistoricalContracts.Count -eq 21
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C34FFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C34FFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C34FFile `
...
$rejectedC33OStatePath = Resolve-C34FFile `
...
$rejectedC33PStatePath = Resolve-C34FFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C34FFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C34FFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C34FFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C34FFile `
...
$rejectedC33UStatePath = Resolve-C34FFile `
...
$rejectedC33VStatePath = Resolve-C34FFile `
...
$rejectedC33WStatePath = Resolve-C34FFile `
...
$rejectedC33XStatePath = Resolve-C34FFile `
...
$rejectedC33YStatePath = Resolve-C34FFile `
...
$rejectedC33ZStatePath = Resolve-C34FFile `
...
$rejectedC34AStatePath = Resolve-C34FFile `
...
$rejectedC34BStatePath = Resolve-C34FFile `
...
$rejectedC34CStatePath = Resolve-C34FFile `
...
$rejectedC34DStatePath = Resolve-C34FFile `
...
$rejectedC34EStatePath = Resolve-C34FFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C34FFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C34FFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C34FFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C34FFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C34F founder launcher'
$wrapperPath = Resolve-C34FFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C34FFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C34F interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C34FFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C34F predeclared source-cycle owner'
$resultRetentionPath = Resolve-C34FFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C34FPowerShellOwner -Path $launcherPath -Label 'C34F founder launcher'
Assert-C34FPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C34FPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C34F interrupted-postbuild recovery owner'
Assert-C34FPowerShellOwner `
  -Path $cycleOwnerPath `
...
Assert-C34F -Condition (
  ...
  $cycleOwner.IndexOf(
    '$manifestRelative = "$evidenceRelative/source-manifest-c34f-registry-2646.txt"',
    [StringComparison]::Ordinal
  ) -ge 0 -and
  $cycleOwner.IndexOf('C34F source-cycle prerequisite preflight passed: exact state, registry-2646 manifest and focused manifest paths exist; testsStarted=false.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $cycleOwner.IndexOf('registryEntryCount = 2646', [StringComparison]::Ordinal) -ge 0 -and
  $cycleOwner.IndexOf('5DB74D0BDBEAAB4B9A57D414CA56C90906E368033AF46E429010ADAE7B743C9B', [StringComparison]::Ordinal) -ge 0 -and
...
$gateNeedle = 'scripts/check-uaw-c34f-r60-70-authentication-no-regression-release-readiness.ps1'
...
Assert-C34F -Condition (
  ...
  $launcher.IndexOf('C34F r60.70 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C34F founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C34F binding, gate order or single appbundle owner changed.'
Assert-C34F -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C34FFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C34FFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C34FFile `
...
$googleStatePath = Resolve-C34FFile `
...
Assert-C34FSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C34FFile `
...
Assert-C34FSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C34FFile `
...
Write-Output (
  'C34F r60.70 no-regression release gate passed: ' +
