...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33y.json',
...
function Assert-C33Y {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33Y r60.63 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33YFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33Y -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33YSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33Y -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33YPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33Y -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33YManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33Y -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33YFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33Y -Condition (
...
$ticketId = 'UAW-C33Y-R60-63-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33YFile `
  -Path 'config/uaw-c33y-r60-63-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33Y ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33YSanitizedText -Text $ticketRaw -Label 'C33Y ticket'
Assert-C33Y -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '2577A633CBB7181B6E0C7E20B2E92B0DC152626E70219A0E713430866B612B45'
...
Assert-C33Y -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.63' -and
  [string]$ticket.candidate.versionCode -ceq '2026081363' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C33X-R60-62-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  [string]$ticket.classificationReason -match 'REG2649' -and
...
$uploadRunbookPath = Resolve-C33YFile `
  -Path 'docs/quality/UAW-C33Y-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33Y pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33YSanitizedText -Text $uploadRunbook -Label 'C33Y upload runbook'
Assert-C33Y -Condition (
...
$resolvedStatePath = Resolve-C33YFile -Path $StatePath -Label 'C33Y state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33YSanitizedText -Text $stateRaw -Label 'C33Y state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33YFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33Y aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33YSanitizedText -Text $aggregateRaw -Label 'C33Y aggregate'
...
Assert-C33Y -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.63' -and
  [string]$state.candidate.versionCode -ceq '2026081363' -and
...
Assert-C33Y -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33Y-R60-63-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.63' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081363'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33YFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33Y pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33YFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33Y end-to-end founder authorization'
Assert-C33Y -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33Y-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33Y-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33Y-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33Y -Condition (
  $historicalContracts.Count -eq 14 -and
  $aggregateHistoricalContracts.Count -eq 14
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33YFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33YFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33YFile `
...
$rejectedC33OStatePath = Resolve-C33YFile `
...
$rejectedC33PStatePath = Resolve-C33YFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33YFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33YFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C33YFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C33YFile `
...
$rejectedC33UStatePath = Resolve-C33YFile `
...
$rejectedC33VStatePath = Resolve-C33YFile `
...
$rejectedC33WStatePath = Resolve-C33YFile `
...
$rejectedC33XStatePath = Resolve-C33YFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33YFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33YFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33YFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33YFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33Y founder launcher'
$wrapperPath = Resolve-C33YFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33YFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33Y interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C33YFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C33Y predeclared source-cycle owner'
$resultRetentionPath = Resolve-C33YFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33YPowerShellOwner -Path $launcherPath -Label 'C33Y founder launcher'
Assert-C33YPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33YPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33Y interrupted-postbuild recovery owner'
Assert-C33YPowerShellOwner `
  -Path $cycleOwnerPath `
...
$gateNeedle = 'scripts/check-uaw-c33y-r60-63-authentication-no-regression-release-readiness.ps1'
...
Assert-C33Y -Condition (
  ...
  $launcher.IndexOf('C33Y r60.63 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33Y founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33Y binding, gate order or single appbundle owner changed.'
Assert-C33Y -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33YFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33YFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33YFile `
...
$googleStatePath = Resolve-C33YFile `
...
Assert-C33YSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33YFile `
...
Assert-C33YSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C33YFile `
...
if ($cycles -eq 0) {
  ...
  Assert-C33YManifestCurrent -ManifestPath $manifestPath
  Assert-C33Y -Condition (
    (Get-FileHash -Algorithm SHA256 -LiteralPath $focusedManifestPath).Hash -ceq
      [string]$state.sourceQualification.focusedManifestSha256
...
Write-Output (
  'C33Y r60.63 no-regression release gate passed: ' +
