...
function Assert-C34KTransition {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34K lifecycle transition rejected: $Message"
  }
}

function Resolve-C34KTransitionPath {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34KTransition -Condition (
...
function Assert-C34KCommonParity {
  param(
...
function Set-C34KMachineAndDisposition {
  param(
    [Parameter(Mandatory)][object]$State,
    [Parameter(Mandatory)][object]$Aggregate,
...
$stateFile = Resolve-C34KTransitionPath -Path $StatePath -Label 'detailed state'
$stateOriginal = [IO.File]::ReadAllText($stateFile)
$state = $stateOriginal | ConvertFrom-Json
$aggregateFile = Resolve-C34KTransitionPath `
...
Assert-C34KCommonParity -State $state -Aggregate $aggregate
if ($InjectFailureAfterStateCommit) {
  Assert-C34KTransition -Condition $FixtureMode `
    -Message 'failure injection is fixture-only.'
  $fixturePrefix = [IO.Path]::GetFullPath((Join-Path $root 'tmp/c34k-lifecycle-fixtures'))
  Assert-C34KTransition -Condition (
    $stateFile.StartsWith($fixturePrefix, [StringComparison]::OrdinalIgnoreCase)
  ) -Message 'failure injection state is outside the C34K fixture root.'
}
...
switch ($Transition) {
  'founder-inputs-validated' {
    ...
    Set-C34KMachineAndDisposition -State $state -Aggregate $aggregate `
  ...
  'build-start' {
    ...
    Set-C34KMachineAndDisposition -State $state -Aggregate $aggregate -Value $value
  ...
  'prebuild-failed' {
    ...
    if (-not $FixtureMode) {
      [void](Resolve-C34KTransitionPath -Path $EvidencePath `
  ...
  'build-succeeded' {
    ...
    if (-not $FixtureMode) {
      $artifactFile = Resolve-C34KTransitionPath -Path $ArtifactPath -Label 'sealed AAB'
      Assert-C34KTransition -Condition (
      ...
      [void](Resolve-C34KTransitionPath -Path $ArtifactProvenance -Label 'AAB provenance')
    }
  ...
  'build-failed' {
    ...
    if (-not $FixtureMode) {
      [void](Resolve-C34KTransitionPath -Path $EvidencePath -Label 'build failure evidence')
    }
  ...
  'upload-succeeded' {
    ...
    if (-not $FixtureMode) {
      [void](Resolve-C34KTransitionPath -Path $EvidencePath -Label 'Play activation evidence')
    }
  ...
  'install-succeeded' {
    ...
    if (-not $FixtureMode) {
      [void](Resolve-C34KTransitionPath -Path $EvidencePath -Label 'OPPO update evidence')
    }
  ...
  'device-accepted' {
    ...
    if (-not $FixtureMode) {
      [void](Resolve-C34KTransitionPath -Path $EvidencePath -Label 'device journey evidence')
    }
  ...
  'reject' {
    ...
    if (-not $FixtureMode) {
      [void](Resolve-C34KTransitionPath -Path $EvidencePath -Label 'rejection evidence')
    }
    Set-C34KMachineAndDisposition -State $state -Aggregate $aggregate `
...
$suffix = '.c34k-transition-' + $PID + '-' + [Guid]::NewGuid().ToString('N')
...
try {
  ...
  $stateCommitted = $true
  if ($InjectFailureAfterStateCommit) {
    throw 'C34K fixture injected failure after detailed-state commit.'
  }
...
Write-Output (
  'C34K lifecycle transition passed: ' +
