...
$repairBinding = @(
  $policy.productionGitDiscipline.continuationBindings | Where-Object {
    [string]$_.id -ceq 'integration_repair_store_buy_conflict_v1_20260904'
  }
)
...
$expectedBootstrapOwners = @(
  'config/codex-development-regression-registry.json',
  'config/codex-subagent-coordination-policy.json',
  'docs/quality/STORE-BUY-PRE-INTEGRATION-RECONCILIATION-20260904.md',
  'docs/quality/UAW-INTEGRATION-REPAIR-STORE-BUY-V1-20260904.md',
...
if ($RequireQualifiedGraph) {
  ...
  Assert-RepairContract (
    ...
    $subject -cmatch '^repair\(store-buy-conflict-repair-v1-20260904\): .+'
