...
$repairBinding = @(
  $policy.productionGitDiscipline.continuationBindings | Where-Object {
    [string]$_.id -ceq 'integration_repair_store_buy_conflict_v2_20260904'
  }
)
...
$expectedBootstrapOwners = @(
  'config/codex-development-regression-registry.json',
  'config/codex-subagent-coordination-policy.json',
  'docs/quality/STORE-BUY-V2-PRE-INTEGRATION-RECONCILIATION-20260904.md',
  'docs/quality/UAW-INTEGRATION-REPAIR-STORE-BUY-V2-20260904.md',
...
if ($RequireQualifiedGraph) {
  ...
  Assert-RepairContract (
    ...
    $subject -cmatch '^repair\(store-buy-conflict-repair-v2-20260904\): .+'
