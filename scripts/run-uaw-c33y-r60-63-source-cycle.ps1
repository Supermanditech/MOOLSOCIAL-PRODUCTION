...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33Y source cycle requires PowerShell 7 or newer.'
}
...
if ($branch -cne 'remediation/prototype-conformance-2026-07-20' -or $head -cne 'f6dfe7587aa02d782e94282d14af8bafff48ded0') {
  throw 'C33Y source cycle repository identity changed.'
}

$ticketId = 'UAW-C33Y-R60-63-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRelative = 'artifacts/quality/uaw-c33y-r60-63-authentication-no-regression-preparation-20260816-01'
$evidenceRoot = Join-Path $root $evidenceRelative
$manifestRelative = "$evidenceRelative/source-manifest-c33y-registry-2621.txt"
$focusedRelative = "$evidenceRelative/focused-test-manifest-c33y.txt"
$stateRelative = 'config/successor-aab-regression-hard-gate-state-c33y.json'
$statePath = Join-Path $root $stateRelative
$prefix = 'c33y-cycle-{0:d2}' -f $Cycle
$logs = [ordered]@{
...
foreach ($path in $logs.Values) {
  if (Test-Path -LiteralPath $path) { throw 'C33Y source-cycle evidence already exists; overwrite is forbidden.' }
}
foreach ($path in @($statePath, (Join-Path $root $manifestRelative), (Join-Path $root $focusedRelative))) {
  if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'C33Y source-cycle prerequisite is missing.' }
}
$state = Get-Content -Raw -LiteralPath $statePath | ConvertFrom-Json
if (
  [string]$state.ticketId -cne $ticketId -or
  [string]$state.machineState -cne 'prebuild_composition_registered_two_fresh_cycles_required' -or
...
) { throw 'C33Y source-cycle held-authority state changed.' }

function ConvertTo-C33YLines {
  param([object[]]$Values)
  return @($Values | ForEach-Object { if ($null -eq $_) { '' } else { [string]$_ } })
}

function Assert-C33YSanitized {
  param([string[]]$Lines)
  $text = $Lines -join "`n"
  foreach ($pattern in @(
    ...
    if ([regex]::IsMatch($text, $pattern)) { throw 'C33Y source-cycle output violated the sanitized evidence boundary.' }
  }
}

function Write-C33YLog {
  param([string]$Path, [string[]]$Lines)
  Assert-C33YSanitized -Lines $Lines
  [IO.File]::WriteAllLines($Path, $Lines, [Text.UTF8Encoding]::new($false))
}

function Invoke-C33YNative {
  ...
  $lines = ConvertTo-C33YLines -Values $raw
  Write-C33YLog -Path $LogPath -Lines $lines
  if ($code -ne 0) { throw 'C33Y native source-cycle step failed; inspect only the sanitized retained log.' }
  return $lines
}

$staticLines = [Collections.Generic.List[string]]::new()
function Add-C33YStatic {
  param([object[]]$Values)
  foreach ($line in (ConvertTo-C33YLines -Values $Values)) { $staticLines.Add($line) }
}

$manifestOwner = Join-Path $root 'scripts/new-c30v-source-manifest.ps1'
Add-C33YStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Add-C33YStatic -Values @(& (Join-Path $root 'scripts/check-codex-development-regression-memory.ps1') -Phase implementation)
Add-C33YStatic -Values @(& (Join-Path $root 'scripts/check-mvp-scope-gate-state.ps1') -StatePath 'config/mvp-scope-gate-state.json' -CandidateId $ticketId -RequireExecutionAuthorized -RepositoryRoot $root)
Add-C33YStatic -Values @(& (Join-Path $root 'scripts/check-approved-ui-locks.ps1') -RepositoryRoot $root)

$candidateGate = Join-Path $root 'scripts/check-uaw-c33y-r60-63-authentication-no-regression-release-readiness.ps1'
Push-Location 'C:\WINDOWS\system32'
try {
  Add-C33YStatic -Values @(& $candidateGate -Phase source -StatePath $stateRelative -RepositoryRoot $root)
} finally { Pop-Location }
...
Add-C33YStatic -Values $winRaw
if ($winCode -ne 0) { throw 'C33Y Windows PowerShell source gate failed.' }

$pwsh = (Get-Command pwsh -ErrorAction Stop).Source
$flutterRunner = Join-Path $root 'tmp/run-c30t-authoritative-flutter-manifest-audit.ps1'
$flutterLines = Invoke-C33YNative -File $pwsh -Arguments @(
...
$analyzerLines = Invoke-C33YNative -File $flutter -Arguments @('analyze', '--no-pub') `
  -WorkingDirectory (Join-Path $root 'apps/mobile') -LogPath $logs.analyzer
$npm = (Get-Command npm.cmd -ErrorAction Stop).Source
$backendTypecheckLines = Invoke-C33YNative -File $npm -Arguments @('run', 'typecheck') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTypecheck
$backendTestLines = Invoke-C33YNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTest
$webTestLines = Invoke-C33YNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'apps/web') -LogPath $logs.webTest

Add-C33YStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Write-C33YLog -Path $logs.static -Lines $staticLines.ToArray()
...
if (-not $flutterMatch.Success) { throw 'C33Y Flutter summary marker is missing.' }
...
) { throw 'C33Y source-cycle bounded result counts changed.' }

$summary = [ordered]@{
  schemaVersion = 1
  ticketId = $ticketId
  cycle = $Cycle
  registryEntryCount = 2621
  registrySha256 = '97F9C1CD3C99A86FBE1CB9A33433C127695C817FD64C477338C6446E517DFC00'
...
Assert-C33YSanitized -Lines @($summaryText)
[IO.File]::WriteAllText($logs.summary, $summaryText, [Text.UTF8Encoding]::new($false))
Write-Output "C33Y source cycle passed: cycle=$Cycle; Flutter=501/3/0; backend=537; web=8; sourceUnchanged=true; PlayWrites=0."
