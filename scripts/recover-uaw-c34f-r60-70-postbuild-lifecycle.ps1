...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c34f.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34F postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34FRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34F existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34FRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34FRecovery -Condition (
...
function Write-C34FRecoveryState {
  param(
  ...
  Assert-C34FRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c34f-r60-70-authentication-no-regression-preparation-20260817-01'
$stateFile = Resolve-C34FRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34FRecoveryPath `
...
$transientGoogleServices = Resolve-C34FRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34FRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.70-2026081370-release.aab"
$sealedPath = Resolve-C34FRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34FRecoveryPath `
...
Assert-C34FRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34FRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.70' -and
  [string]$provenance.versionCode -ceq '2026081370' -and
...
$buildLog = Resolve-C34FRecoveryPath `
...
Assert-C34FRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34FRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34f-recovery-state-write'
  Write-C34FRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34f-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c34f-r60-70-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C34F existing-AAB postbuild lifecycle recovery passed: ' +
