...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34B source cycle requires PowerShell 7 or newer.'
}
...
if ($branch -cne 'remediation/prototype-conformance-2026-07-20' -or $head -cne 'f6dfe7587aa02d782e94282d14af8bafff48ded0') {
  throw 'C34B source cycle repository identity changed.'
}

$ticketId = 'UAW-C34B-R60-66-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRelative = 'artifacts/quality/uaw-c34b-r60-66-authentication-no-regression-preparation-20260816-01'
$evidenceRoot = Join-Path $root $evidenceRelative
$manifestRelative = "$evidenceRelative/source-manifest-c34b-registry-2628.txt"
$focusedRelative = "$evidenceRelative/focused-test-manifest-c34b.txt"
$stateRelative = 'config/successor-aab-regression-hard-gate-state-c34b.json'
$statePath = Join-Path $root $stateRelative
$prefix = 'c34b-cycle-{0:d2}' -f $Cycle
$logs = [ordered]@{
...
foreach ($path in $logs.Values) {
  if (Test-Path -LiteralPath $path) { throw 'C34B source-cycle evidence already exists; overwrite is forbidden.' }
}
foreach ($path in @($statePath, (Join-Path $root $manifestRelative), (Join-Path $root $focusedRelative))) {
  if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'C34B source-cycle prerequisite is missing.' }
}
...
) { throw 'C34B source-cycle held-authority state changed.' }

function ConvertTo-C34BLines {
  param([object[]]$Values)
  return @($Values | ForEach-Object { if ($null -eq $_) { '' } else { [string]$_ } })
}

function Assert-C34BSanitized {
  param([string[]]$Lines)
  $text = $Lines -join "`n"
  foreach ($pattern in @(
    ...
    if ([regex]::IsMatch($text, $pattern)) { throw 'C34B source-cycle output violated the sanitized evidence boundary.' }
  }
}

function Write-C34BLog {
  param([string]$Path, [string[]]$Lines)
  Assert-C34BSanitized -Lines $Lines
  [IO.File]::WriteAllLines($Path, $Lines, [Text.UTF8Encoding]::new($false))
}

function Invoke-C34BNative {
  ...
  $lines = ConvertTo-C34BLines -Values $raw
  Write-C34BLog -Path $LogPath -Lines $lines
  if ($code -ne 0) { throw 'C34B native source-cycle step failed; inspect only the sanitized retained log.' }
  return $lines
}

$staticLines = [Collections.Generic.List[string]]::new()
function Add-C34BStatic {
  param([object[]]$Values)
  foreach ($line in (ConvertTo-C34BLines -Values $Values)) { $staticLines.Add($line) }
}

$manifestOwner = Join-Path $root 'scripts/new-c30v-source-manifest.ps1'
Add-C34BStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Add-C34BStatic -Values @(& (Join-Path $root 'scripts/check-codex-development-regression-memory.ps1') -Phase implementation)
Add-C34BStatic -Values @(& (Join-Path $root 'scripts/check-mvp-scope-gate-state.ps1') -StatePath 'config/mvp-scope-gate-state.json' -CandidateId $ticketId -RequireExecutionAuthorized -RepositoryRoot $root)
Add-C34BStatic -Values @(& (Join-Path $root 'scripts/check-approved-ui-locks.ps1') -RepositoryRoot $root)

$candidateGate = Join-Path $root 'scripts/check-uaw-c34b-r60-66-authentication-no-regression-release-readiness.ps1'
Push-Location 'C:\WINDOWS\system32'
try {
  Add-C34BStatic -Values @(& $candidateGate -Phase source -StatePath $stateRelative -RepositoryRoot $root)
} finally { Pop-Location }
...
Add-C34BStatic -Values $winRaw
if ($winCode -ne 0) { throw 'C34B Windows PowerShell source gate failed.' }

$pwsh = (Get-Command pwsh -ErrorAction Stop).Source
$flutterRunner = Join-Path $root 'tmp/run-c30t-authoritative-flutter-manifest-audit.ps1'
$flutterLines = Invoke-C34BNative -File $pwsh -Arguments @(
...
$analyzerLines = Invoke-C34BNative -File $flutter -Arguments @('analyze', '--no-pub') `
  -WorkingDirectory (Join-Path $root 'apps/mobile') -LogPath $logs.analyzer
$npm = (Get-Command npm.cmd -ErrorAction Stop).Source
$backendTypecheckLines = Invoke-C34BNative -File $npm -Arguments @('run', 'typecheck') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTypecheck
$backendTestLines = Invoke-C34BNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTest
$webTestLines = Invoke-C34BNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'apps/web') -LogPath $logs.webTest

Add-C34BStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Write-C34BLog -Path $logs.static -Lines $staticLines.ToArray()
...
if (-not $flutterMatch.Success) { throw 'C34B Flutter summary marker is missing.' }
...
) { throw 'C34B source-cycle bounded result counts changed.' }

$summary = [ordered]@{
  schemaVersion = 1
  ticketId = $ticketId
  cycle = $Cycle
  registryEntryCount = 2628
  registrySha256 = 'C9FC574A63646867EC8AACFBDE1EB6337FD1D88DBFE74025DF773117E5A9FC4E'
...
Assert-C34BSanitized -Lines @($summaryText)
[IO.File]::WriteAllText($logs.summary, $summaryText, [Text.UTF8Encoding]::new($false))
Write-Output "C34B source cycle passed: cycle=$Cycle; Flutter=501/3/0; backend=537; web=8; sourceUnchanged=true; PlayWrites=0."
