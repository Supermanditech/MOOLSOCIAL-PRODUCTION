...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33o.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33O postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33ORecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33O existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33ORecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33ORecovery -Condition (
...
function Write-C33ORecoveryState {
  param(
  ...
  Assert-C33ORecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$candidateId =
  'UAW-C33O-R60-53-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRoot =
  'artifacts/quality/uaw-c33o-r60-53-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33ORecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33ORecoveryPath `
...
$transientGoogleServices = Resolve-C33ORecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33ORecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.53-2026081353-release.aab"
$sealedPath = Resolve-C33ORecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33ORecoveryPath `
...
Assert-C33ORecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33ORecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.53' -and
  [string]$provenance.versionCode -ceq '2026081353' -and
...
$buildLog = Resolve-C33ORecoveryPath `
...
Assert-C33ORecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33ORecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33o-recovery-state-write'
  Write-C33ORecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33o-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33o-r60-53-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33O existing-AAB postbuild lifecycle recovery passed: ' +
