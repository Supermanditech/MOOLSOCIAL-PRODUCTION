...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c34b.json',
...
function Assert-C34B {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34B r60.66 no-regression release gate rejected: $Message"
  }
}

function Resolve-C34BFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C34B -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C34BSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C34B -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C34BPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C34B -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C34BManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C34B -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C34BFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C34B -Condition (
...
$ticketId = 'UAW-C34B-R60-66-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C34BFile `
  -Path 'config/uaw-c34b-r60-66-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C34B ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C34BSanitizedText -Text $ticketRaw -Label 'C34B ticket'
Assert-C34B -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '6E40F96B4A1AC57386D6851DF1F3B612D0000F90EDF333CB15FB88E31E6D17ED'
...
Assert-C34B -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.66' -and
  [string]$ticket.candidate.versionCode -ceq '2026081366' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C34A-R60-65-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  [string]$ticket.classificationReason -match 'REG2652' -and
  [string]$ticket.classificationReason -match 'REG2653' -and
  [string]$ticket.classificationReason -match 'REG2654' -and
  [string]$ticket.classificationReason -match 'REG2655' -and
  [string]$ticket.classificationReason -match 'REG2656' -and
  [string]$ticket.classificationReason -match 'REG2657' -and
  ...
  @($ticket.minimumCompleteScope) -contains
    'reset_all_three_current_candidate_founder_configuration_qualification_flags_false_before_source_seal_and_require_source_build_gate_parity_with_the_launcher_preprompt_predicates' -and
  @($ticket.minimumCompleteScope) -contains
...
$uploadRunbookPath = Resolve-C34BFile `
  -Path 'docs/quality/UAW-C34B-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C34B pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C34BSanitizedText -Text $uploadRunbook -Label 'C34B upload runbook'
Assert-C34B -Condition (
...
$resolvedStatePath = Resolve-C34BFile -Path $StatePath -Label 'C34B state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C34BSanitizedText -Text $stateRaw -Label 'C34B state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C34BFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C34B aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C34BSanitizedText -Text $aggregateRaw -Label 'C34B aggregate'
...
Assert-C34B -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.66' -and
  [string]$state.candidate.versionCode -ceq '2026081366' -and
...
Assert-C34B -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34B-R60-66-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.66' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081366'
...
$browserQualificationPath = Resolve-C34BFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C34B pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C34BFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C34B end-to-end founder authorization'
Assert-C34B -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C34B-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C34B-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C34B-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C34B -Condition (
  $historicalContracts.Count -eq 17 -and
  $aggregateHistoricalContracts.Count -eq 17
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C34BFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C34BFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C34BFile `
...
$rejectedC33OStatePath = Resolve-C34BFile `
...
$rejectedC33PStatePath = Resolve-C34BFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C34BFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C34BFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C34BFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C34BFile `
...
$rejectedC33UStatePath = Resolve-C34BFile `
...
$rejectedC33VStatePath = Resolve-C34BFile `
...
$rejectedC33WStatePath = Resolve-C34BFile `
...
$rejectedC33XStatePath = Resolve-C34BFile `
...
$rejectedC33YStatePath = Resolve-C34BFile `
...
$rejectedC33ZStatePath = Resolve-C34BFile `
...
$rejectedC34AStatePath = Resolve-C34BFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C34BFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C34BFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C34BFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C34BFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C34B founder launcher'
$wrapperPath = Resolve-C34BFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C34BFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C34B interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C34BFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C34B predeclared source-cycle owner'
$resultRetentionPath = Resolve-C34BFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C34BPowerShellOwner -Path $launcherPath -Label 'C34B founder launcher'
Assert-C34BPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C34BPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C34B interrupted-postbuild recovery owner'
Assert-C34BPowerShellOwner `
  -Path $cycleOwnerPath `
...
$gateNeedle = 'scripts/check-uaw-c34b-r60-66-authentication-no-regression-release-readiness.ps1'
...
Assert-C34B -Condition (
  ...
  $launcher.IndexOf('C34B r60.66 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C34B founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C34B binding, gate order or single appbundle owner changed.'
Assert-C34B -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C34BFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C34BFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C34BFile `
...
$googleStatePath = Resolve-C34BFile `
...
Assert-C34BSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C34BFile `
...
Assert-C34BSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C34BFile `
...
Write-Output (
  'C34B r60.66 no-regression release gate passed: ' +
