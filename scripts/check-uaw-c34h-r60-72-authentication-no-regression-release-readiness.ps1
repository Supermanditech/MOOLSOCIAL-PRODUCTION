...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c34h.json',
...
function Assert-C34H {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34H r60.72 no-regression release gate rejected: $Message"
  }
}

function Resolve-C34HFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C34H -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C34HSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C34H -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C34HPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C34H -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C34HManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C34H -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C34HFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C34H -Condition (
...
Assert-C34HSanitizedText -Text $ticketRaw -Label 'C34H ticket'
Assert-C34H -Condition (
...
Assert-C34H -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.72' -and
  [string]$ticket.candidate.versionCode -ceq '2026081372' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C34G-R60-71-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  [string]$ticket.classificationReason -match 'REG2694' -and
  [string]$ticket.classificationReason -match 'REG2696' -and
  [string]$ticket.classificationReason -match 'atomically persist' -and
  ...
  @($ticket.minimumCompleteScope) -contains
    'bind_the_cycle_runner_manifestRelative_to_the_final_registry_numbered_source_manifest_assert_exact_parity_from_the_candidate_gate_and_pass_a_prerequisite_path_only_preflight_before_source_seal' -and
  @($ticket.minimumCompleteScope) -contains
    'assert_real_state_real_aggregate_and_all_four_transition_fixtures_have_the_same_final_historical_candidate_count_and_last_rejected_candidate_identity_before_the_phase_matrix' -and
  @($ticket.minimumCompleteScope) -contains
    'prequalify_browser_control_to_open_one_fresh_known_Internal_Testing_tab_without_openTabs_history_or_unrelated_tab_enumeration_and_emit_only_query_free_allowlisted_Play_host_path_state' -and
  @($ticket.minimumCompleteScope) -contains
    'reject_the_candidate_and_register_an_exact_repair_ticket_before_retry_if_any_historical_or_new_regression_occurs' -and
  @($ticket.minimumCompleteScope) -contains
    'run_both_fresh_independent_complete_source_cycles_while_real_state_and_aggregate_remain_0_of_2_and_all_release_authorities_remain_held' -and
  @($ticket.minimumCompleteScope) -contains
    'verify_both_retained_cycle_summaries_independently_then_atomically_persist_2_of_2_and_both_evidence_paths_before_any_source_replay_or_founder_prompt_authority' -and
  @($ticket.minimumCompleteScope) -contains
    'assert_nested_candidate_actionCounts_buildResult_PlayResult_installResult_and_release_authority_parity_before_seal_and_after_every_lifecycle_transition' -and
...
$uploadRunbookPath = Resolve-C34HFile `
  -Path 'docs/quality/UAW-C34H-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260817.md' `
  -Label 'C34H pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C34HSanitizedText -Text $uploadRunbook -Label 'C34H upload runbook'
Assert-C34H -Condition (
  ...
  [regex]::IsMatch($uploadRunbook, 'Those\s+two\s+lifecycle\s+transitions\s+cannot\s+be\s+combined[.]') -and
  $uploadRunbook.Contains('founder_inputs_validated_single_aab_build_required') -and
  $uploadRunbook.Contains('Do not persist cycle 1 as `1/2`.') -and
  $uploadRunbook.Contains('atomically persist `2/2`') -and
  [regex]::IsMatch($uploadRunbook, 'The wrapper''s\s+existing\s+`build`\s+replay\s+requires\s+that\s+exact') -and
  $uploadRunbook.Contains('must not enumerate or emit open tabs') -and
  [regex]::IsMatch($uploadRunbook, 'query-free\s+allowlisted\s+Play\s+host/path') -and
  $uploadRunbook.Contains('REG-20260817-2664-C34E-PREUPLOAD-RAW-CHROME-TAB-URL-QUERY-EXPOSURE-RECURRENCE') -and
  [regex]::IsMatch($uploadRunbook, 'Testing\s*>\s*Internal\s+testing') -and
  [regex]::IsMatch($uploadRunbook, 'No\s+post-seal\s+source,\s+registry,\s+ticket,\s+runbook\s+or\s+gate\s+mutation\s+is\s+allowed[.]')
) -Message 'pre-sealed Internal Testing upload workflow boundary changed.'

$resolvedStatePath = Resolve-C34HFile -Path $StatePath -Label 'C34H state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C34HSanitizedText -Text $stateRaw -Label 'C34H state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C34HFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C34H aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C34HSanitizedText -Text $aggregateRaw -Label 'C34H aggregate'
...
Assert-C34H -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.72' -and
  [string]$state.candidate.versionCode -ceq '2026081372' -and
...
Assert-C34H -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34H-R60-72-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.72' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081372'
...
$browserQualificationPath = Resolve-C34HFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C34H pre-sealed browser qualification evidence'
$browserQualification = Get-Content -Raw -LiteralPath $browserQualificationPath
Assert-C34HSanitizedText `
  -Text $browserQualification `
  -Label 'C34H pre-sealed browser qualification evidence'
Assert-C34H -Condition (
  $browserQualification.Contains('must not call or emit `openTabs()`') -and
  $browserQualification.Contains('one fresh tab at the known') -and
  $browserQualification.Contains('query-free allowlisted Play host/path') -and
  [regex]::IsMatch($browserQualification, 'No\s+new\s+C34H\s+browser\s+action\s+was\s+performed[.]') -and
  $browserQualification.Contains('The reused C34G qualification is bound to regression registry 2,664') -and
  $browserQualification.Contains('REG-20260817-2664-C34E-PREUPLOAD-RAW-CHROME-TAB-URL-QUERY-EXPOSURE-RECURRENCE')
) -Message 'fresh known Play tab and raw-tab-inventory privacy prevention changed.'
$founderAuthorizationPath = Resolve-C34HFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C34H end-to-end founder authorization'
Assert-C34H -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C34H-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260817.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C34H-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260817.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C34H-END-TO-END-FOUNDER-AUTHORIZATION-20260817.md' -and
...
Assert-C34H -Condition (
  $historicalContracts.Count -eq 23 -and
  $aggregateHistoricalContracts.Count -eq 23
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C34HFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C34HFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C34HFile `
...
$rejectedC33OStatePath = Resolve-C34HFile `
...
$rejectedC33PStatePath = Resolve-C34HFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C34HFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C34HFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C34HFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C34HFile `
...
$rejectedC33UStatePath = Resolve-C34HFile `
...
$rejectedC33VStatePath = Resolve-C34HFile `
...
$rejectedC33WStatePath = Resolve-C34HFile `
...
$rejectedC33XStatePath = Resolve-C34HFile `
...
$rejectedC33YStatePath = Resolve-C34HFile `
...
$rejectedC33ZStatePath = Resolve-C34HFile `
...
$rejectedC34AStatePath = Resolve-C34HFile `
...
$rejectedC34BStatePath = Resolve-C34HFile `
...
$rejectedC34CStatePath = Resolve-C34HFile `
...
$rejectedC34DContract = $historicalContracts[19]
$rejectedC34DStatePath = Resolve-C34HFile `
  -Path ([string]$rejectedC34DContract.statePath) `
  -Label 'rejected r60.68 state'
$rejectedC34DState = Get-Content -Raw -LiteralPath $rejectedC34DStatePath | ConvertFrom-Json
Assert-C34H -Condition (
  [string]$rejectedC34DContract.versionName -ceq '1.0.0-r60.68' -and
  [string]$rejectedC34DContract.versionCode -ceq '2026081368' -and
  [string]$rejectedC34DContract.disposition -ceq 'rejected' -and
  [string]$rejectedC34DContract.machineState -ceq
    'prebuild_rejected_sealed_cycle_runner_manifest_path_mismatch_successor_required' -and
  [int]$rejectedC34DContract.buildCount -eq 0 -and
  [int]$rejectedC34DContract.uploadCount -eq 0 -and
  [int]$rejectedC34DContract.installCount -eq 0 -and
  [int]$rejectedC34DContract.deviceAcceptanceCount -eq 0 -and
  -not [bool]$rejectedC34DContract.artifactReusable -and
  [string]$rejectedC34DState.machineState -ceq
    'prebuild_rejected_sealed_cycle_runner_manifest_path_mismatch_successor_required' -and
  [string]$rejectedC34DState.rejection.reasonRegistryIds[0] -ceq
    'REG-20260816-2662-C34D-SEALED-CYCLE-RUNNER-STALE-SOURCE-MANIFEST-REGISTRY-PATH' -and
  -not [bool]$rejectedC34DState.rejection.hiddenInputsEntered -and
  -not [bool]$rejectedC34DState.rejection.buildAuthorityConsumed -and
  [int]$rejectedC34DState.sourceQualification.completedIdenticalCycles -eq 0 -and
  [int]$rejectedC34DState.actionCounts.build -eq 0 -and
  [int]$rejectedC34DState.actionCounts.upload -eq 0 -and
  [int]$rejectedC34DState.actionCounts.install -eq 0 -and
  [int]$rejectedC34DState.actionCounts.deviceAcceptance -eq 0 -and
  [string]$aggregateHistoricalContracts[19].machineState -ceq
    'prebuild_rejected_sealed_cycle_runner_manifest_path_mismatch_successor_required'
) -Message 'rejected r60.68 sealed cycle-runner manifest-path disposition or 0/0/0/0 truth changed.'

$rejectedC34EContract = $historicalContracts[20]
$rejectedC34EStatePath = Resolve-C34HFile `
  -Path ([string]$rejectedC34EContract.statePath) `
  -Label 'rejected r60.69 state'
$rejectedC34EState = Get-Content -Raw -LiteralPath $rejectedC34EStatePath |
  ConvertFrom-Json
Assert-C34H -Condition (
  [string]$rejectedC34EContract.versionName -ceq '1.0.0-r60.69' -and
  [string]$rejectedC34EContract.versionCode -ceq '2026081369' -and
  [string]$rejectedC34EContract.disposition -ceq 'rejected' -and
  [string]$rejectedC34EContract.machineState -ceq
    'preupload_rejected_raw_authenticated_tab_query_recurrence_successor_required' -and
  [int]$rejectedC34EContract.buildCount -eq 1 -and
  [int]$rejectedC34EContract.uploadCount -eq 0 -and
  [int]$rejectedC34EContract.installCount -eq 0 -and
  [int]$rejectedC34EContract.deviceAcceptanceCount -eq 0 -and
  -not [bool]$rejectedC34EContract.artifactReusable -and
  [string]$rejectedC34EContract.artifactSha256 -ceq
    '8FDB9A5CC6D925A7D5E79FEBA169703A5177D5B4E6B5BFB767E06DC0D7542213' -and
  [string]$rejectedC34EState.machineState -ceq
    'preupload_rejected_raw_authenticated_tab_query_recurrence_successor_required' -and
  [string]$rejectedC34EState.buildAuthorization -ceq 'consumed' -and
  [string]$rejectedC34EState.uploadAuthorization -ceq 'rejected_candidate' -and
  [int]$rejectedC34EState.actionCounts.build -eq 1 -and
  [int]$rejectedC34EState.actionCounts.upload -eq 0 -and
  [int]$rejectedC34EState.actionCounts.install -eq 0 -and
  [int]$rejectedC34EState.actionCounts.deviceAcceptance -eq 0 -and
  [string]$rejectedC34EState.buildResult.artifactSha256 -ceq
    '8FDB9A5CC6D925A7D5E79FEBA169703A5177D5B4E6B5BFB767E06DC0D7542213' -and
  [string]$rejectedC34EState.rejection.reasonRegistryIds[1] -ceq
    'REG-20260817-2664-C34E-PREUPLOAD-RAW-CHROME-TAB-URL-QUERY-EXPOSURE-RECURRENCE' -and
  [bool]$rejectedC34EState.privacyBoundary.privateLinksObserved -and
  [bool]$rejectedC34EState.privacyBoundary.tokenOrAttestationPayloadObserved -and
  [string]$aggregateHistoricalContracts[20].machineState -ceq
    'preupload_rejected_raw_authenticated_tab_query_recurrence_successor_required' -and
  [string]$aggregateHistoricalContracts[20].aabSha256 -ceq
    '8FDB9A5CC6D925A7D5E79FEBA169703A5177D5B4E6B5BFB767E06DC0D7542213'
) -Message 'rejected r60.69 raw authenticated-tab query recurrence or 1/0/0/0 truth changed.'

$rejectedC34FContract = $historicalContracts[21]
$rejectedC34FStatePath = Resolve-C34HFile `
  -Path ([string]$rejectedC34FContract.statePath) `
  -Label 'rejected r60.70 state'
$rejectedC34FState = Get-Content -Raw -LiteralPath $rejectedC34FStatePath |
  ConvertFrom-Json
Assert-C34H -Condition (
  [string]$rejectedC34FContract.versionName -ceq '1.0.0-r60.70' -and
  [string]$rejectedC34FContract.versionCode -ceq '2026081370' -and
  [string]$rejectedC34FContract.disposition -ceq 'rejected' -and
  [string]$rejectedC34FContract.machineState -ceq
    'prebuild_rejected_cycle1_authoritative_flutter_audit_unset_event_successor_required' -and
  [int]$rejectedC34FContract.buildCount -eq 0 -and
  [int]$rejectedC34FContract.uploadCount -eq 0 -and
  [int]$rejectedC34FContract.installCount -eq 0 -and
  [int]$rejectedC34FContract.deviceAcceptanceCount -eq 0 -and
  -not [bool]$rejectedC34FContract.artifactReusable -and
  $null -eq $rejectedC34FContract.artifactSha256 -and
  [string]$rejectedC34FState.machineState -ceq
    'prebuild_rejected_cycle1_authoritative_flutter_audit_unset_event_successor_required' -and
  [string]$rejectedC34FState.buildAuthorization -ceq 'rejected_source_cycle_failure' -and
  [string]$rejectedC34FState.rejection.reasonRegistryIds[0] -ceq
    'REG-20260817-2676-C34F-CYCLE1-AUTHORITATIVE-FLUTTER-AUDIT-UNSET-EVENT' -and
  [int]$rejectedC34FState.sourceQualification.completedIdenticalCycles -eq 0 -and
  [int]$rejectedC34FState.actionCounts.build -eq 0 -and
  [int]$rejectedC34FState.actionCounts.upload -eq 0 -and
  [int]$rejectedC34FState.actionCounts.install -eq 0 -and
  [int]$rejectedC34FState.actionCounts.deviceAcceptance -eq 0 -and
  [string]$aggregateHistoricalContracts[21].machineState -ceq
    'prebuild_rejected_cycle1_authoritative_flutter_audit_unset_event_successor_required'
) -Message 'rejected r60.70 authoritative Flutter unset-event disposition or 0/0/0/0 truth changed.'

$rejectedC34GContract = $historicalContracts[22]
$rejectedC34GStatePath = Resolve-C34HFile `
  -Path ([string]$rejectedC34GContract.statePath) `
  -Label 'rejected r60.71 state'
$rejectedC34GState = Get-Content -Raw -LiteralPath $rejectedC34GStatePath |
  ConvertFrom-Json
Assert-C34H -Condition (
  [string]$rejectedC34GContract.versionName -ceq '1.0.0-r60.71' -and
  [string]$rejectedC34GContract.versionCode -ceq '2026081371' -and
  [string]$rejectedC34GContract.disposition -ceq 'rejected' -and
  [string]$rejectedC34GContract.machineState -ceq
    'prebuild_rejected_premature_cycle1_persistence_successor_required' -and
  [int]$rejectedC34GContract.buildCount -eq 0 -and
  [int]$rejectedC34GContract.uploadCount -eq 0 -and
  [int]$rejectedC34GContract.installCount -eq 0 -and
  [int]$rejectedC34GContract.deviceAcceptanceCount -eq 0 -and
  -not [bool]$rejectedC34GContract.artifactReusable -and
  $null -eq $rejectedC34GContract.artifactSha256 -and
  [string]$rejectedC34GState.machineState -ceq
    'prebuild_rejected_premature_cycle1_persistence_successor_required' -and
  [string]$rejectedC34GState.rejection.reasonRegistryIds[0] -ceq
    'REG-20260817-2694-C34G-POST-CYCLE1-PREMATURE-INTERMEDIATE-STATE-PERSISTENCE' -and
  [int]$rejectedC34GState.sourceQualification.completedIdenticalCycles -eq 1 -and
  @($rejectedC34GState.sourceQualification.cycleEvidence).Count -eq 1 -and
  -not [bool]$rejectedC34GState.rejection.cycleOneQualificationReusable -and
  -not [bool]$rejectedC34GState.rejection.cycleTwoStarted -and
  [int]$rejectedC34GState.actionCounts.build -eq 0 -and
  [int]$rejectedC34GState.actionCounts.upload -eq 0 -and
  [int]$rejectedC34GState.actionCounts.install -eq 0 -and
  [int]$rejectedC34GState.actionCounts.deviceAcceptance -eq 0 -and
  [string]$aggregateHistoricalContracts[22].machineState -ceq
    'prebuild_rejected_premature_cycle1_persistence_successor_required'
) -Message 'rejected r60.71 premature cycle-1 persistence, nonreuse or authoritative 0/0/0/0 truth changed.'
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C34HFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C34HFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C34HFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C34HFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C34H founder launcher'
$wrapperPath = Resolve-C34HFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C34HFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C34H interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C34HFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C34H predeclared source-cycle owner'
$resultRetentionPath = Resolve-C34HFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
$loopControlGatePath = Resolve-C34HFile `
  -Path 'scripts/check-uaw-c34h-c34g-authoritative-flutter-nonobject-loop-control-replay.ps1' `
  -Label 'C34H successor replay of qualified C34G Flutter loop-control prevention'
Assert-C34HPowerShellOwner -Path $launcherPath -Label 'C34H founder launcher'
Assert-C34HPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C34HPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C34H interrupted-postbuild recovery owner'
Assert-C34HPowerShellOwner `
  -Path $cycleOwnerPath `
...
Assert-C34HPowerShellOwner `
  ...
  -Label 'founder launcher result-retention owner'
Assert-C34HPowerShellOwner `
  -Path $loopControlGatePath `
  -Label 'C34H successor replay of qualified C34G Flutter loop-control prevention'
& $loopControlGatePath -RepositoryRoot $root | Out-Null
...
Assert-C34H -Condition (
  ...
  $cycleOwner.IndexOf('[switch]$PrerequisiteOnly', [StringComparison]::Ordinal) -ge 0 -and
  $cycleOwner.IndexOf(
    '$manifestRelative = "$evidenceRelative/source-manifest-c34h-registry-2674.txt"',
    [StringComparison]::Ordinal
  ) -ge 0 -and
  $cycleOwner.IndexOf('C34H source-cycle prerequisite preflight passed: exact state, registry-2674 manifest and focused manifest paths exist; testsStarted=false.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $cycleOwner.IndexOf('registryEntryCount = 2674', [StringComparison]::Ordinal) -ge 0 -and
  $cycleOwner.IndexOf('B6BCD44D884BEFB8C3B031DA79B76785EA875197C6E69C72618FFE58A055748E', [StringComparison]::Ordinal) -ge 0 -and
...
$gateNeedle = 'scripts/check-uaw-c34h-r60-72-authentication-no-regression-release-readiness.ps1'
...
Assert-C34H -Condition (
  ...
  $launcher.IndexOf('C34H r60.72 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C34H founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C34H binding, gate order or single appbundle owner changed.'
Assert-C34H -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C34HFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C34HFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C34HFile `
...
$googleStatePath = Resolve-C34HFile `
...
Assert-C34HSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C34HFile `
...
Assert-C34HSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C34HFile `
...
if ($cycles -eq 0) {
  if ([string]$state.machineState -ceq 'prebuild_composition_registered_two_fresh_cycles_required') {
    Assert-C34H -Condition (
      ...
      [int]$aggregate.candidate.deviceAcceptanceCount -eq 0 -and
      $null -eq $aggregate.candidate.aabSha256 -and
      [string]$aggregate.candidate.disposition -ceq
        'prebuild_composition_registered_two_fresh_cycles_required' -and
      [int]$state.buildResult.buildCount -eq 0 -and
      [int]$state.playResult.uploadCount -eq 0 -and
      [int]$state.installResult.installCount -eq 0
...
if ($Phase -ceq 'preprompt') {
  ...
  Assert-C34H -Condition (
    ...
    [int]$aggregate.candidate.uploadCount -eq 0 -and
    [int]$aggregate.candidate.installCount -eq 0 -and
    $null -eq $aggregate.candidate.aabSha256
...
if ($Phase -ceq 'postbuild') {
  Assert-C34H -Condition (
    ...
    [bool]$state.buildResult.mergedReleaseManifestProved -and
...
if ($Phase -ceq 'postupload') {
  Assert-C34H -Condition (
    ...
    [int]$state.actionCounts.install -eq 0 -and
    -not [string]::IsNullOrWhiteSpace([string]$state.playResult.evidencePath) -and
    [int]$aggregate.candidate.buildCount -eq 1 -and
...
Write-Output (
  'C34H r60.72 no-regression release gate passed: ' +
