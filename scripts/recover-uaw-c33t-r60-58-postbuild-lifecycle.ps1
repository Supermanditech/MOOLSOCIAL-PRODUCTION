...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33t.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33T postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33TRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33T existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33TRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33TRecovery -Condition (
...
function Write-C33TRecoveryState {
  param(
  ...
  Assert-C33TRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$candidateId =
  'UAW-C33T-R60-58-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRoot =
  'artifacts/quality/uaw-c33t-r60-58-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33TRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33TRecoveryPath `
...
$transientGoogleServices = Resolve-C33TRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33TRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.58-2026081358-release.aab"
$sealedPath = Resolve-C33TRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33TRecoveryPath `
...
Assert-C33TRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33TRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.58' -and
  [string]$provenance.versionCode -ceq '2026081358' -and
...
$buildLog = Resolve-C33TRecoveryPath `
...
Assert-C33TRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33TRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33t-recovery-state-write'
  Write-C33TRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33t-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33t-r60-58-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33T existing-AAB postbuild lifecycle recovery passed: ' +
