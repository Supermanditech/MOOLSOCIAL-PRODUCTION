...
function Assert-C34J {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34J r60.74 lifecycle-atomic release gate rejected: $Message"
  }
}

function Resolve-C34JFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C34J -Condition (
...
function Assert-C34JManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  $rows = @(Get-Content -LiteralPath $ManifestPath)
  foreach ($row in $rows) {
    $match = [regex]::Match($row, '^([0-9A-F]{64})  (.+)$')
    Assert-C34J -Condition $match.Success -Message 'source manifest row is malformed.'
    $owner = Resolve-C34JFile -Path $match.Groups[2].Value -Label 'source owner'
    Assert-C34J -Condition (
...
$stateFile = Resolve-C34JFile -Path $StatePath -Label 'detailed state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34JFile `
...
$ticketFile = Resolve-C34JFile `
  -Path 'config/uaw-c34j-r60-74-release-lifecycle-atomic-parity-play-oppo-acceptance-ticket.json' `
  -Label 'selected ticket'
$ticket = Get-Content -Raw -LiteralPath $ticketFile | ConvertFrom-Json
$scopeFile = Resolve-C34JFile -Path 'config/mvp-scope-gate-state.json' -Label 'MVP state'
$scope = Get-Content -Raw -LiteralPath $scopeFile | ConvertFrom-Json
$registryFile = Resolve-C34JFile `
...
Assert-C34J -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.74' -and
  [string]$ticket.candidate.versionCode -ceq '2026081374' -and
...
Assert-C34J -Condition (
  ...
  [string]$selected.manifestPath -ceq
    'config/uaw-c34j-r60-74-release-lifecycle-atomic-parity-play-oppo-acceptance-ticket.json' -and
  ...
  [string]$scope.authorization.evidence -ceq
    'docs/quality/UAW-C34J-END-TO-END-FOUNDER-AUTHORIZATION-20260817.md' -and
...
Assert-C34J -Condition (
  [string]$state.contractId -ceq
    'MOOLSOCIAL-C34J-R60-74-RELEASE-LIFECYCLE-ATOMIC-PARITY-STATE-001' -and
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34J-R60-74-RELEASE-LIFECYCLE-ATOMIC-PARITY-AGGREGATE-001' -and
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.74' -and
  [string]$state.candidate.versionCode -ceq '2026081374' -and
...
$priorStateFile = Resolve-C34JFile `
  -Path 'config/successor-aab-regression-hard-gate-state-c34i.json' `
  -Label 'C34I rejected detailed state'
$priorAggregateFile = Resolve-C34JFile `
  -Path 'config/successor-aab-regression-hard-gate-aggregate-c34i.json' `
  -Label 'C34I rejected aggregate state'
...
Assert-C34J -Condition (
  [string]$priorState.machineState -ceq
    'postbuild_rejected_detailed_aggregate_build_authority_mirror_mismatch_successor_required' -and
  [string]$priorAggregate.machineState -ceq [string]$priorState.machineState -and
  [int]$priorState.actionCounts.build -eq 1 -and
  ...
  [string]$priorState.buildResult.artifactSha256 -ceq
    'FDC10D458B003280E78BBF3B11519352406ABD90AF7412204C632F38E73DAA4E' -and
  ...
  [int]$lastStateHistory.buildCount -eq 1 -and
...
) -Message 'C34I rejection or successor historical binding changed.'
...
foreach ($token in @(
  ...
  'check-uaw-c34j-r60-74-release-lifecycle-atomic-parity-readiness.ps1',
  'invoke-release-lifecycle-transition-c34j.ps1',
  ...
  ) -Message "generic wrapper C34J binding changed: $token"
}
...
foreach ($owner in $requiredOwners) {
  [void](Resolve-C34JFile -Path $owner -Label 'required C34J owner')
}

$mvpGate = Resolve-C34JFile `
...
$blockerGate = Resolve-C34JFile `
  -Path 'scripts/check-uaw-c33g-fix4-unresolved-acceptance-blocker-pre-aab-ledger.ps1' `
  -Label 'C33G blocker gate'
$blockerPhase = if ($Phase -cin @('postinstall', 'journey')) {
  'postinstall'
} else { 'prebuild' }
& $blockerGate -CandidateId $ticketId -CandidateVersionCode '2026081374' `
...
if ($Phase -ceq 'source') {
  if ($cycles -eq 0) {
    ...
    if ([string]$state.machineState -ceq
      ...
      Assert-C34JManifestCurrent -ManifestPath $manifest
      $focused = Resolve-C34JFile `
      ...
      throw 'C34J r60.74 lifecycle-atomic release gate rejected: unsupported cycles-zero machine state.'
    }
  } elseif ($cycles -eq 2) {
    Assert-C34J -Condition (
    ...
    throw 'C34J r60.74 lifecycle-atomic release gate rejected: intermediate cycle persistence is forbidden.'
  }
}
...
Write-Output (
  'C34J r60.74 lifecycle-atomic release gate passed: ' +
