...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33n.json',
...
function Assert-C33N {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33N r60.52 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33NFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33N -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33NSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33N -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33NPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33N -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33NManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33N -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33NFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33N -Condition (
...
$ticketId = 'UAW-C33N-R60-52-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33NFile `
  -Path 'config/uaw-c33n-r60-52-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33N ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33NSanitizedText -Text $ticketRaw -Label 'C33N ticket'
Assert-C33N -Condition (
...
Assert-C33N -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.52' -and
  [string]$ticket.candidate.versionCode -ceq '2026081352' -and
...
$resolvedStatePath = Resolve-C33NFile -Path $StatePath -Label 'C33N state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33NSanitizedText -Text $stateRaw -Label 'C33N state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33NFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33N aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33NSanitizedText -Text $aggregateRaw -Label 'C33N aggregate'
...
Assert-C33N -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.52' -and
  [string]$state.candidate.versionCode -ceq '2026081352' -and
...
Assert-C33N -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33N-R60-52-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.52' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081352'
...
Assert-C33N -Condition (
  $historicalContracts.Count -eq 3 -and
  $aggregateHistoricalContracts.Count -eq 3
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33NFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33NFile -Path ([string]$rejectedContract.statePath) `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33NFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33NFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33NFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33NFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33N founder launcher'
$wrapperPath = Resolve-C33NFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33NFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33N interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33NFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33NPowerShellOwner -Path $launcherPath -Label 'C33N founder launcher'
Assert-C33NPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33NPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33N interrupted-postbuild recovery owner'
Assert-C33NPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33n-r60-52-authentication-no-regression-release-readiness.ps1'
...
Assert-C33N -Condition (
  ...
  $launcher.IndexOf(
    "throw 'C33N founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33N binding, gate order or single appbundle owner changed.'
Assert-C33N -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33NFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33NFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33NFile `
...
$googleStatePath = Resolve-C33NFile `
...
Assert-C33NSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33NFile `
...
Assert-C33NSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33N -Condition $sourceQualified `
  ...
  Assert-C33NManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33NFile `
...
Write-Output (
  'C33N r60.52 no-regression release gate passed: ' +
