...
$storeBuyFinalSeptember12 = (
  ...
  $ProductionWorkId -ceq 'store-buy-final-v8-20260913' -and
  $ProductionTicketId -ceq 'UAW-INTEGRATION-STORE-BUY-FINAL-V8-20260913' -and
  $AgentRole -ceq 'primary' -and $AgentTask -ceq '/root' -and
  (ConvertTo-ProductionForwardPath $root) -ceq
    'C:/GUARANTEED OUTCOME/MOOLSOCIAL-WORKTREE-INTEGRATION-store-buy-final-v8-20260913'
)
...
if ($ProductionLane -ceq 'baseline') {
  ...
  if ($ProductionPhase -ceq 'integration_admission_authorize') {
    if ($storeBuyFinalAdmission) {
      ...
      Assert-Coordination (
        $IntegrationTargetWorkId -ceq 'store-buy-final-v8-20260913' -and
        $IntegrationTargetTicketId -ceq 'UAW-INTEGRATION-STORE-BUY-FINAL-V8-20260913' -and
        (ConvertTo-ProductionForwardPath $IntegrationTargetRoot) -ceq
          'C:/GUARANTEED OUTCOME/MOOLSOCIAL-WORKTREE-INTEGRATION-store-buy-final-v8-20260913' -and
      ...
      Assert-Coordination ($LASTEXITCODE -eq 0 -and $targetBranch.Count -eq 1 -and
        $targetBranch[0] -ceq 'integration/moolsocial/store-buy-final-v8-20260913') 'Final integration branch changed.'
      ...
      $targetRemote = @(& git -C $IntegrationTargetRoot ls-remote --heads origin `
        'refs/heads/integration/moolsocial/store-buy-final-v8-20260913')
      Assert-Coordination ($LASTEXITCODE -eq 0 -and $targetRemote.Count -eq 0) 'Final integration remote already exists.'
      Write-Output 'merge(store-buy-final-v8-20260913): integrate qualified Store and Buy'
    } else {
