...
$cursorStorefrontOwners = @(
  ...
  # Founder-selected C12/C13 right-side Add: migrate the old bottom-row assertion.
...
$cursorReviewQualificationOwners = @(
  ...
  'docs/quality/CURSOR-BUY-REDMI-CHILD-FIXES-20260924.zip',
...
$storeBuyFinalSeptember12 = (
  ...
  $ProductionWorkId -ceq 'store-buy-final-v31-20260916' -and
  $ProductionTicketId -ceq 'UAW-INTEGRATION-STORE-BUY-FINAL-V31-20260916' -and
  $AgentRole -ceq 'primary' -and $AgentTask -ceq '/root' -and
...
'@).Replace("`r`n", "`n") + "`n", '')
  ...
  if ($ProductionPhase -ceq 'integration_admission_authorize') {
    if ($counterSaleAdmission) {
      ...
      Assert-Coordination (
        $IntegrationTargetWorkId -ceq 'store-buy-final-v31-20260916' -and
        $IntegrationTargetTicketId -ceq 'UAW-INTEGRATION-STORE-BUY-FINAL-V31-20260916' -and
        (ConvertTo-ProductionForwardPath $IntegrationTargetRoot) -ceq
          'C:/GUARANTEED OUTCOME/MOOLSOCIAL-WORKTREE-INTEGRATION-store-buy-final-v31-20260916' -and
        (Test-Path -LiteralPath $IntegrationTargetRoot -PathType Container)
      ) 'Final integration target identity changed.'
      $targetBranch = @(& git -C $IntegrationTargetRoot branch --show-current)
      Assert-Coordination ($LASTEXITCODE -eq 0 -and $targetBranch.Count -eq 1 -and
        $targetBranch[0] -ceq 'integration/moolsocial/store-buy-final-v31-20260916') 'Final integration branch changed.'
      $targetHead = @(& git -C $IntegrationTargetRoot rev-parse HEAD)
      Assert-Coordination ($LASTEXITCODE -eq 0 -and $targetHead.Count -eq 1 -and
      ...
      $targetRemote = @(& git -C $IntegrationTargetRoot ls-remote --heads origin `
        'refs/heads/integration/moolsocial/store-buy-final-v31-20260916')
      Assert-Coordination ($LASTEXITCODE -eq 0 -and $targetRemote.Count -eq 0) 'Final integration remote already exists.'
      Write-Output 'merge(store-buy-final-v31-20260916): integrate qualified Store and Buy'
    } else {
