...
function Assert-ProductionManagedWorktreesClean {
  param([switch]$IndependentCursorAudit)
  ...
  if ($IndependentCursorAudit) {
    ...
    }).Count -eq 0) 'An admitted Cursor input cannot be treated as independent.'
  }
...
$storeBuyFinalSeptember12 = (
  ...
  $ProductionWorkId -ceq 'store-buy-final-v29-20260916' -and
  $ProductionTicketId -ceq 'UAW-INTEGRATION-STORE-BUY-FINAL-V29-20260916' -and
  $AgentRole -ceq 'primary' -and $AgentTask -ceq '/root' -and
  (ConvertTo-ProductionForwardPath $root) -ceq
    'C:/GUARANTEED OUTCOME/MOOLSOCIAL-WORKTREE-INTEGRATION-store-buy-final-v29-20260916'
)
...
if ($ProductionLane -ceq 'baseline') {
  ...
  if ($ProductionPhase -ceq 'integration_admission_authorize') {
    if ($storeBuyFinalAdmission) {
      $qualifiedSource = '92d90843316490cbfd5511ad0d176187943ea70c'
      ...
      Assert-Coordination (
        $IntegrationTargetWorkId -ceq 'store-buy-final-v29-20260916' -and
        $IntegrationTargetTicketId -ceq 'UAW-INTEGRATION-STORE-BUY-FINAL-V29-20260916' -and
        (ConvertTo-ProductionForwardPath $IntegrationTargetRoot) -ceq
          'C:/GUARANTEED OUTCOME/MOOLSOCIAL-WORKTREE-INTEGRATION-store-buy-final-v29-20260916' -and
      ...
      Assert-Coordination ($LASTEXITCODE -eq 0 -and $targetBranch.Count -eq 1 -and
        $targetBranch[0] -ceq 'integration/moolsocial/store-buy-final-v29-20260916') 'Final integration branch changed.'
      ...
      $targetRemote = @(& git -C $IntegrationTargetRoot ls-remote --heads origin `
        'refs/heads/integration/moolsocial/store-buy-final-v29-20260916')
      Assert-Coordination ($LASTEXITCODE -eq 0 -and $targetRemote.Count -eq 0) 'Final integration remote already exists.'
      Write-Output 'merge(store-buy-final-v29-20260916): integrate qualified Store and Buy'
    } else {
