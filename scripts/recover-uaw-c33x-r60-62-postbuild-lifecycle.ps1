...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33x.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33X postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33XRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33X existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33XRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33XRecovery -Condition (
...
function Write-C33XRecoveryState {
  param(
  ...
  Assert-C33XRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33x-r60-62-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33XRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33XRecoveryPath `
...
$transientGoogleServices = Resolve-C33XRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33XRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.62-2026081362-release.aab"
$sealedPath = Resolve-C33XRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33XRecoveryPath `
...
Assert-C33XRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33XRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.62' -and
  [string]$provenance.versionCode -ceq '2026081362' -and
...
$buildLog = Resolve-C33XRecoveryPath `
...
Assert-C33XRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33XRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33x-recovery-state-write'
  Write-C33XRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33x-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33x-r60-62-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33X existing-AAB postbuild lifecycle recovery passed: ' +
