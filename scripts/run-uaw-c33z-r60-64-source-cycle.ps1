...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33Z source cycle requires PowerShell 7 or newer.'
}
...
if ($branch -cne 'remediation/prototype-conformance-2026-07-20' -or $head -cne 'f6dfe7587aa02d782e94282d14af8bafff48ded0') {
  throw 'C33Z source cycle repository identity changed.'
}

$ticketId = 'UAW-C33Z-R60-64-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRelative = 'artifacts/quality/uaw-c33z-r60-64-authentication-no-regression-preparation-20260816-01'
$evidenceRoot = Join-Path $root $evidenceRelative
$manifestRelative = "$evidenceRelative/source-manifest-c33z-registry-2622.txt"
$focusedRelative = "$evidenceRelative/focused-test-manifest-c33z.txt"
$stateRelative = 'config/successor-aab-regression-hard-gate-state-c33z.json'
$statePath = Join-Path $root $stateRelative
$prefix = 'c33z-cycle-{0:d2}' -f $Cycle
$logs = [ordered]@{
...
foreach ($path in $logs.Values) {
  if (Test-Path -LiteralPath $path) { throw 'C33Z source-cycle evidence already exists; overwrite is forbidden.' }
}
foreach ($path in @($statePath, (Join-Path $root $manifestRelative), (Join-Path $root $focusedRelative))) {
  if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'C33Z source-cycle prerequisite is missing.' }
}
...
) { throw 'C33Z source-cycle held-authority state changed.' }

function ConvertTo-C33ZLines {
  param([object[]]$Values)
  return @($Values | ForEach-Object { if ($null -eq $_) { '' } else { [string]$_ } })
}

function Assert-C33ZSanitized {
  param([string[]]$Lines)
  $text = $Lines -join "`n"
  foreach ($pattern in @(
    ...
    if ([regex]::IsMatch($text, $pattern)) { throw 'C33Z source-cycle output violated the sanitized evidence boundary.' }
  }
}

function Write-C33ZLog {
  param([string]$Path, [string[]]$Lines)
  Assert-C33ZSanitized -Lines $Lines
  [IO.File]::WriteAllLines($Path, $Lines, [Text.UTF8Encoding]::new($false))
}

function Invoke-C33ZNative {
  ...
  $lines = ConvertTo-C33ZLines -Values $raw
  Write-C33ZLog -Path $LogPath -Lines $lines
  if ($code -ne 0) { throw 'C33Z native source-cycle step failed; inspect only the sanitized retained log.' }
  return $lines
}

$staticLines = [Collections.Generic.List[string]]::new()
function Add-C33ZStatic {
  param([object[]]$Values)
  foreach ($line in (ConvertTo-C33ZLines -Values $Values)) { $staticLines.Add($line) }
}

$manifestOwner = Join-Path $root 'scripts/new-c30v-source-manifest.ps1'
Add-C33ZStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Add-C33ZStatic -Values @(& (Join-Path $root 'scripts/check-codex-development-regression-memory.ps1') -Phase implementation)
Add-C33ZStatic -Values @(& (Join-Path $root 'scripts/check-mvp-scope-gate-state.ps1') -StatePath 'config/mvp-scope-gate-state.json' -CandidateId $ticketId -RequireExecutionAuthorized -RepositoryRoot $root)
Add-C33ZStatic -Values @(& (Join-Path $root 'scripts/check-approved-ui-locks.ps1') -RepositoryRoot $root)

$candidateGate = Join-Path $root 'scripts/check-uaw-c33z-r60-64-authentication-no-regression-release-readiness.ps1'
Push-Location 'C:\WINDOWS\system32'
try {
  Add-C33ZStatic -Values @(& $candidateGate -Phase source -StatePath $stateRelative -RepositoryRoot $root)
} finally { Pop-Location }
...
Add-C33ZStatic -Values $winRaw
if ($winCode -ne 0) { throw 'C33Z Windows PowerShell source gate failed.' }

$pwsh = (Get-Command pwsh -ErrorAction Stop).Source
$flutterRunner = Join-Path $root 'tmp/run-c30t-authoritative-flutter-manifest-audit.ps1'
$flutterLines = Invoke-C33ZNative -File $pwsh -Arguments @(
...
$analyzerLines = Invoke-C33ZNative -File $flutter -Arguments @('analyze', '--no-pub') `
  -WorkingDirectory (Join-Path $root 'apps/mobile') -LogPath $logs.analyzer
$npm = (Get-Command npm.cmd -ErrorAction Stop).Source
$backendTypecheckLines = Invoke-C33ZNative -File $npm -Arguments @('run', 'typecheck') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTypecheck
$backendTestLines = Invoke-C33ZNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTest
$webTestLines = Invoke-C33ZNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'apps/web') -LogPath $logs.webTest

Add-C33ZStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Write-C33ZLog -Path $logs.static -Lines $staticLines.ToArray()
...
if (-not $flutterMatch.Success) { throw 'C33Z Flutter summary marker is missing.' }
...
) { throw 'C33Z source-cycle bounded result counts changed.' }

$summary = [ordered]@{
  schemaVersion = 1
  ticketId = $ticketId
  cycle = $Cycle
  registryEntryCount = 2622
  registrySha256 = 'D1212DAD542CD4DBEB6E3A459B134133532887E9FD0C01A9AF1EBD62D584C8DA'
...
Assert-C33ZSanitized -Lines @($summaryText)
[IO.File]::WriteAllText($logs.summary, $summaryText, [Text.UTF8Encoding]::new($false))
Write-Output "C33Z source cycle passed: cycle=$Cycle; Flutter=501/3/0; backend=537; web=8; sourceUnchanged=true; PlayWrites=0."
