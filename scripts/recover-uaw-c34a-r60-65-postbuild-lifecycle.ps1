...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c34a.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34A postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34ARecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34A existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34ARecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34ARecovery -Condition (
...
function Write-C34ARecoveryState {
  param(
  ...
  Assert-C34ARecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c34a-r60-65-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C34ARecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34ARecoveryPath `
...
$transientGoogleServices = Resolve-C34ARecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34ARecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.65-2026081365-release.aab"
$sealedPath = Resolve-C34ARecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34ARecoveryPath `
...
Assert-C34ARecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34ARecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.65' -and
  [string]$provenance.versionCode -ceq '2026081365' -and
...
$buildLog = Resolve-C34ARecoveryPath `
...
Assert-C34ARecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34ARecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34a-recovery-state-write'
  Write-C34ARecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34a-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c34a-r60-65-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C34A existing-AAB postbuild lifecycle recovery passed: ' +
