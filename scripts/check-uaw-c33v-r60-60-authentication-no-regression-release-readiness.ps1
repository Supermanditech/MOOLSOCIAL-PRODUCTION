...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33v.json',
...
function Assert-C33V {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33V r60.60 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33VFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33V -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33VSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33V -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33VPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33V -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33VManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33V -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33VFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33V -Condition (
...
$ticketId = 'UAW-C33V-R60-60-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33VFile `
  -Path 'config/uaw-c33v-r60-60-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33V ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33VSanitizedText -Text $ticketRaw -Label 'C33V ticket'
Assert-C33V -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '7ED3E1BED16F7585201BACCD1A00FCE8DF91A03D8DA7B0B260B22A0BEA1A6A7A'
...
Assert-C33V -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.60' -and
  [string]$ticket.candidate.versionCode -ceq '2026081360' -and
...
$uploadRunbookPath = Resolve-C33VFile `
  -Path 'docs/quality/UAW-C33V-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33V pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33VSanitizedText -Text $uploadRunbook -Label 'C33V upload runbook'
Assert-C33V -Condition (
...
$resolvedStatePath = Resolve-C33VFile -Path $StatePath -Label 'C33V state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33VSanitizedText -Text $stateRaw -Label 'C33V state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33VFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33V aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33VSanitizedText -Text $aggregateRaw -Label 'C33V aggregate'
...
Assert-C33V -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.60' -and
  [string]$state.candidate.versionCode -ceq '2026081360' -and
...
Assert-C33V -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33V-R60-60-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.60' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081360'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33VFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33V pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33VFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33V end-to-end founder authorization'
Assert-C33V -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33V-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33V-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33V-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33V -Condition (
  $historicalContracts.Count -eq 11 -and
  $aggregateHistoricalContracts.Count -eq 11
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33VFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33VFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33VFile `
...
$rejectedC33OStatePath = Resolve-C33VFile `
...
$rejectedC33PStatePath = Resolve-C33VFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33VFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33VFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C33VFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C33VFile `
...
$rejectedC33UStatePath = Resolve-C33VFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33VFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33VFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33VFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33VFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33V founder launcher'
$wrapperPath = Resolve-C33VFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33VFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33V interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33VFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33VPowerShellOwner -Path $launcherPath -Label 'C33V founder launcher'
Assert-C33VPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33VPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33V interrupted-postbuild recovery owner'
Assert-C33VPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33v-r60-60-authentication-no-regression-release-readiness.ps1'
...
Assert-C33V -Condition (
  ...
  $launcher.IndexOf('C33V r60.60 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33V founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33V binding, gate order or single appbundle owner changed.'
Assert-C33V -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33VFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33VFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33VFile `
...
$googleStatePath = Resolve-C33VFile `
...
Assert-C33VSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33VFile `
...
Assert-C33VSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33VManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33VFile `
...
Write-Output (
  'C33V r60.60 no-regression release gate passed: ' +
