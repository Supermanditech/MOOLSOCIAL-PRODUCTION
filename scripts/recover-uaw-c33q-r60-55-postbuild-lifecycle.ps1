...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33q.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33Q postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33QRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33Q existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33QRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33QRecovery -Condition (
...
function Write-C33QRecoveryState {
  param(
  ...
  Assert-C33QRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33q-r60-55-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33QRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33QRecoveryPath `
...
$transientGoogleServices = Resolve-C33QRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33QRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.55-2026081355-release.aab"
$sealedPath = Resolve-C33QRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33QRecoveryPath `
...
Assert-C33QRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33QRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.55' -and
  [string]$provenance.versionCode -ceq '2026081355' -and
...
$buildLog = Resolve-C33QRecoveryPath `
...
Assert-C33QRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33QRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33q-recovery-state-write'
  Write-C33QRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33q-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33q-r60-55-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33Q existing-AAB postbuild lifecycle recovery passed: ' +
