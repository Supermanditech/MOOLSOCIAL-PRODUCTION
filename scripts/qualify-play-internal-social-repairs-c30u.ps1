...
if ($PSVersionTable.PSVersion.Major -lt 7) { throw 'C30U qualification requires PowerShell 7.' }
if (-not $RepositoryRoot) { $RepositoryRoot = Split-Path -Parent $PSScriptRoot }
$root = [IO.Path]::GetFullPath($RepositoryRoot).TrimEnd([char[]]@('\', '/'))
$mobileRoot = Join-Path $root 'apps/mobile'
$backendRoot = Join-Path $root 'backend/functions'
$statePath = Join-Path $root 'config/play-internal-aab-regression-gate-state-c30u.json'
$aggregatePath = Join-Path $root 'config/play-internal-social-repairs-acceptance-gate-state-c30u.json'
$artifactRelative = 'artifacts/quality/uaw-c30u-post-r60-45-social-repairs-play-internal-acceptance-20260813-01'
...
function Assert-C30UQualification {
  param([Parameter(Mandatory)][bool]$Condition, [Parameter(Mandatory)][string]$Message)
  if (-not $Condition) { throw "C30U qualification rejected: $Message" }
}
...
function Write-C30UJson {
  ...
  Assert-C30UQualification -Condition (-not (Test-Path -LiteralPath $temporary)) -Message "stale temporary state exists: $temporary"
...
function Invoke-C30UCaptured {
  ...
  Assert-C30UQualification -Condition (-not (Test-Path -LiteralPath $LogPath)) -Message "qualification log already exists: $LogPath"
...
function Get-C30UArtifactSnapshot {
...
function Get-C30UTapCounts {
  param([Parameter(Mandatory)][string]$Path)
  $passRows = @(Select-String -LiteralPath $Path -Pattern '^(?:#|ℹ) pass ([0-9]+)$')
  $failRows = @(Select-String -LiteralPath $Path -Pattern '^(?:#|ℹ) fail ([0-9]+)$')
  Assert-C30UQualification -Condition ($passRows.Count -ge 1 -and $failRows.Count -ge 1) -Message "TAP summary missing: $Path"
...
function Get-C30UServiceRevision {
  ...
  Assert-C30UQualification -Condition ($LASTEXITCODE -eq 0) -Message "Cloud Run read failed: $Service"
  $value = ($output | Out-String) | ConvertFrom-Json
  $revision = [string]$value.status.latestReadyRevisionName
  $traffic = @($value.status.traffic)
  Assert-C30UQualification -Condition (
...
Assert-C30UQualification -Condition ($qualifierSource -notmatch '&\s+adb\s+devices\s*\|') -Message 'qualifier still pipelines adb devices before native exit capture.'
Assert-C30UQualification -Condition (Test-Path -LiteralPath $artifactRoot -PathType Container) -Message 'evidence root is missing.'
Assert-C30UQualification -Condition (-not (Test-Path -LiteralPath $cyclePath)) -Message "cycle $Cycle evidence already exists and is immutable."
...
Assert-C30UQualification -Condition (
  ...
  [string]$state.buildAuthorization -ceq 'available_once_after_two_identical_cycles_and_Dev_content_deployment' -and
...
Assert-C30UQualification -Condition (
  [string]$aggregate.providerAndHostingBoundary.deploymentState -ceq 'succeeded_exact_Dev_moolSocialContent_only' -and
  [int]$aggregate.providerAndHostingBoundary.deploymentAttemptCount -eq 1 -and
  [int]$aggregate.providerAndHostingBoundary.deploymentCount -eq 1 -and
...
) -Message 'exact Dev moolSocialContent deployment is not sealed.'
Assert-C30UQualification -Condition (
...
if ($Cycle -eq 1) {
  Assert-C30UQualification -Condition (-not (Test-Path -LiteralPath $sourceManifestPath)) -Message 'accepted source manifest already exists before cycle 1.'
  Assert-C30UQualification -Condition ([int]$state.sourceQualification.identicalQualifyingCycles -eq 0) -Message 'cycle count is not zero before cycle 1.'
} else {
  Assert-C30UQualification -Condition (Test-Path -LiteralPath $sourceManifestPath -PathType Leaf) -Message 'cycle 1 source manifest is missing.'
  Assert-C30UQualification -Condition ([int]$state.sourceQualification.identicalQualifyingCycles -eq 1) -Message 'cycle 1 is not sealed before cycle 2.'
}
...
$apkBefore = Get-C30UArtifactSnapshot -Path $releaseApk
$aabBefore = Get-C30UArtifactSnapshot -Path $releaseAab
...
$formatExit = Invoke-C30UCaptured -Command 'dart' -Arguments @(
...
Assert-C30UQualification -Condition ($formatExit -eq 0) -Message "Flutter/Dart format check failed with exit $formatExit."

$analyzeLog = Join-Path $artifactRoot "$logStem-flutter-analyze.log"
$analyzeExit = Invoke-C30UCaptured -Command 'flutter' -Arguments @('analyze') -WorkingDirectory $mobileRoot -LogPath $analyzeLog
Assert-C30UQualification -Condition ($analyzeExit -eq 0) -Message "whole mobile analyzer failed with exit $analyzeExit."

$flutterLog = Join-Path $artifactRoot "$logStem-authoritative-flutter.log"
$flutterExit = Invoke-C30UCaptured -Command 'pwsh' -Arguments @(
...
Assert-C30UQualification -Condition ($flutterExit -eq 0) -Message "authoritative 58-file Flutter audit failed with exit $flutterExit."
...
Assert-C30UQualification -Condition ($flutterSummary.Count -eq 1) -Message 'authoritative Flutter counted summary changed.'

$backendLog = Join-Path $artifactRoot "$logStem-backend-verify.log"
$backendExit = Invoke-C30UCaptured -Command 'npm' -Arguments @('run', 'verify') -WorkingDirectory $backendRoot -LogPath $backendLog
Assert-C30UQualification -Condition ($backendExit -eq 0) -Message "backend verify failed with exit $backendExit."
$backendCounts = Get-C30UTapCounts -Path $backendLog
Assert-C30UQualification -Condition ($backendCounts.Passed -eq 516 -and $backendCounts.Failed -eq 0) -Message 'backend verify is not 516 passed and 0 failed.'

$hostingLog = Join-Path $artifactRoot "$logStem-hosting-tests.log"
$hostingExit = Invoke-C30UCaptured -Command 'node' -Arguments @('--test', 'apps/web/tests/firebase-public-site.test.mjs') -WorkingDirectory $root -LogPath $hostingLog
Assert-C30UQualification -Condition ($hostingExit -eq 0) -Message "Hosting static tests failed with exit $hostingExit."
$hostingCounts = Get-C30UTapCounts -Path $hostingLog
Assert-C30UQualification -Condition ($hostingCounts.Passed -eq 7 -and $hostingCounts.Failed -eq 0) -Message 'Hosting static tests are not 7 passed and 0 failed.'

$configLog = Join-Path $artifactRoot "$logStem-release-config-only.log"
$configExit = Invoke-C30UCaptured -Command 'flutter' -Arguments @(
  'build', 'apk', '--release', '--config-only', '--build-name=1.0.0-r60.46', '--build-number=2026081346'
) -WorkingDirectory $mobileRoot -LogPath $configLog
Assert-C30UQualification -Condition ($configExit -eq 0) -Message "r60.46 release config-only failed with exit $configExit."
$restoreLog = Join-Path $artifactRoot "$logStem-release-registrant-restore.log"
$restoreExit = Invoke-C30UCaptured -Command 'pwsh' -Arguments @(
...
Assert-C30UQualification -Condition ($restoreExit -eq 0) -Message 'release GeneratedPluginRegistrant restore failed.'
Assert-C30UQualification -Condition (
  ...
  $apkBefore -ceq (Get-C30UArtifactSnapshot -Path $releaseApk) -and
  $aabBefore -ceq (Get-C30UArtifactSnapshot -Path $releaseAab)
) -Message 'release config-only changed dependencies or an APK/AAB.'

$gateLogs = [Collections.Generic.List[string]]::new()
foreach ($gate in @(
  'scripts/check-c30u-social-protected-successor.ps1',
  'scripts/check-youtube-embedded-player-android.ps1',
  'scripts/check-user-facing-copy.ps1',
  'scripts/check-play-internal-aab-build-wrapper-c30u.ps1',
  ...
  $gateExit = Invoke-C30UCaptured -Command 'pwsh' -Arguments @('-NoProfile', '-File', (Join-Path $root $gate)) -WorkingDirectory $root -LogPath $gateLog
  Assert-C30UQualification -Condition ($gateExit -eq 0) -Message "$gate failed with exit $gateExit."
  $gateLogs.Add($gateLog)
}

$youtubeRevision = Get-C30UServiceRevision -Service 'youtubeprovider'
$callbackRevision = Get-C30UServiceRevision -Service 'youtubeoauthcallback'
$contentRevision = Get-C30UServiceRevision -Service 'moolsocialcontent'
$chatRevision = Get-C30UServiceRevision -Service 'moolsocialchat'
Assert-C30UQualification -Condition (
...
) -Message 'post-deployment provider revisions differ from the exact C30U boundary.'

$hostingOutput = & firebase hosting:channel:list --site moolsocial-dev-503018 --project moolsocial-dev-503018 --json 2>$null
Assert-C30UQualification -Condition ($LASTEXITCODE -eq 0) -Message 'live Hosting channel read failed.'
...
Assert-C30UQualification -Condition ($live.Count -eq 1) -Message 'live Hosting channel identity changed.'
...
Assert-C30UQualification -Condition ($deviceExit -eq 0) -Message "adb devices failed with exit $deviceExit."
$deviceRows = @($deviceOutput | Select-Object -Skip 1 | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })
Assert-C30UQualification -Condition ($deviceRows.Count -eq 1 -and [string]$deviceRows[0] -match '^2b3e0f71\s+device$') -Message 'exact OPPO is not the sole connected ready device.'
$packageDump = @(& adb -s 2b3e0f71 shell dumpsys package com.moolsocial.app)
Assert-C30UQualification -Condition ($LASTEXITCODE -eq 0) -Message 'OPPO package read failed.'
...
Assert-C30UQualification -Condition ($versionCodeRows.Count -ge 1 -and $versionNameRows.Count -ge 1) -Message 'OPPO Play predecessor version changed before the one authorized update.'
...
$manifestArguments = if ($Cycle -eq 1) {
  @('-NoProfile', '-File', (Join-Path $root 'scripts/new-c30u-source-manifest.ps1'), '-OutputPath', $provisionalManifestRelative, '-RepositoryRoot', $root)
} else {
  @('-NoProfile', '-File', (Join-Path $root 'scripts/new-c30u-source-manifest.ps1'), '-ComparePath', $sourceManifestRelative, '-RepositoryRoot', $root)
}
$manifestExit = Invoke-C30UCaptured -Command 'pwsh' -Arguments $manifestArguments -WorkingDirectory $root -LogPath $manifestLog
Assert-C30UQualification -Condition ($manifestExit -eq 0) -Message "source manifest cycle $Cycle failed with exit $manifestExit."
...
Assert-C30UQualification -Condition ($manifestSummary.Count -eq 1) -Message 'source manifest protected-owner summary changed.'
...
Assert-C30UQualification -Condition (Test-Path -LiteralPath $manifestEvidencePath -PathType Leaf) -Message 'source manifest evidence is missing.'
...
if ($Cycle -eq 1) {
  Assert-C30UQualification -Condition (-not (Test-Path -LiteralPath $sourceManifestPath)) -Message 'accepted-v7 source manifest already exists before promotion.'
...
Assert-C30UQualification -Condition (Test-Path -LiteralPath $sourceManifestPath -PathType Leaf) -Message 'accepted-v7 source manifest is missing.'
...
if ($Cycle -eq 1) {
  ...
  Assert-C30UQualification -Condition (Test-Path -LiteralPath $cycle1Path -PathType Leaf) -Message 'cycle 1 evidence disappeared.'
  ...
  $state.sourceQualification.state = 'passed_two_identical_cycles_after_Dev_content_deployment'
  ...
  $aggregate.sourceQualification.state = 'passed_two_identical_cycles_after_Dev_content_deployment'
...
Write-C30UJson -State $state -Path $statePath -Suffix (".c30u-cycle{0}-write" -f $Cycle)
Write-C30UJson -State $aggregate -Path $aggregatePath -Suffix (".c30u-cycle{0}-write" -f $Cycle)
...
if ($Cycle -eq 2) {
  ...
  Assert-C30UQualification -Condition (-not (Test-Path -LiteralPath $summaryPath)) -Message 'final pre-AAB summary already exists.'
  $summary = [ordered]@{
    schemaVersion = 1
    candidateId = [string]$state.candidate.id
    ...
    providerDeployment = 'moolsocialcontent-00005-lep_at_100_percent'
...
Write-Output "C30U qualifying cycle $Cycle passed: sourceFiles=$manifestFileCount; sourceFingerprint=$manifestHash; Flutter=405+3skip; backend=516; Hosting=7; build/upload/install=0/0/0."
