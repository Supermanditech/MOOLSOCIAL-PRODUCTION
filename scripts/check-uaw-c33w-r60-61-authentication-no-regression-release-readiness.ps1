...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33w.json',
...
function Assert-C33W {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33W r60.61 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33WFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33W -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33WSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33W -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33WPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33W -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33WManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33W -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33WFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33W -Condition (
...
$ticketId = 'UAW-C33W-R60-61-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33WFile `
  -Path 'config/uaw-c33w-r60-61-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33W ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33WSanitizedText -Text $ticketRaw -Label 'C33W ticket'
Assert-C33W -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '233D641692D252C3547B69506AC26BC7416574E7CA55482124409504C97AB879'
...
Assert-C33W -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.61' -and
  [string]$ticket.candidate.versionCode -ceq '2026081361' -and
...
$uploadRunbookPath = Resolve-C33WFile `
  -Path 'docs/quality/UAW-C33W-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33W pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33WSanitizedText -Text $uploadRunbook -Label 'C33W upload runbook'
Assert-C33W -Condition (
...
$resolvedStatePath = Resolve-C33WFile -Path $StatePath -Label 'C33W state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33WSanitizedText -Text $stateRaw -Label 'C33W state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33WFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33W aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33WSanitizedText -Text $aggregateRaw -Label 'C33W aggregate'
...
Assert-C33W -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33W-R60-61-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.61' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081361'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33WFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33W pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33WFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33W end-to-end founder authorization'
Assert-C33W -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33W-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33W-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33W-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33W -Condition (
  $historicalContracts.Count -eq 12 -and
  $aggregateHistoricalContracts.Count -eq 12
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33WFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33WFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33WFile `
...
$rejectedC33OStatePath = Resolve-C33WFile `
...
$rejectedC33PStatePath = Resolve-C33WFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33WFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33WFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C33WFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C33WFile `
...
$rejectedC33UStatePath = Resolve-C33WFile `
...
$rejectedC33VStatePath = Resolve-C33WFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33WFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33WFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33WFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33WFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33W founder launcher'
$wrapperPath = Resolve-C33WFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33WFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33W interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C33WFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C33W predeclared source-cycle owner'
$resultRetentionPath = Resolve-C33WFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33WPowerShellOwner -Path $launcherPath -Label 'C33W founder launcher'
Assert-C33WPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33WPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33W interrupted-postbuild recovery owner'
Assert-C33WPowerShellOwner `
  -Path $cycleOwnerPath `
...
$gateNeedle = 'scripts/check-uaw-c33w-r60-61-authentication-no-regression-release-readiness.ps1'
...
Assert-C33W -Condition (
  ...
  $launcher.IndexOf('C33W r60.61 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33W founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33W binding, gate order or single appbundle owner changed.'
Assert-C33W -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33WFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33WFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33WFile `
...
$googleStatePath = Resolve-C33WFile `
...
Assert-C33WSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33WFile `
...
Assert-C33WSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33WManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33WFile `
...
Write-Output (
  'C33W r60.61 no-regression release gate passed: ' +
