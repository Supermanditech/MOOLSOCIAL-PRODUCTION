...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33s.json',
...
function Assert-C33S {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33S r60.57 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33SFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33S -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33SSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33S -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33SPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33S -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33SManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33S -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33SFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33S -Condition (
...
$ticketId = 'UAW-C33S-R60-57-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33SFile `
  -Path 'config/uaw-c33s-r60-57-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33S ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33SSanitizedText -Text $ticketRaw -Label 'C33S ticket'
Assert-C33S -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '4CCF50B5BB5091F827C309765DA745B26B78F7353973A6C93900D7C2F6E24118'
...
Assert-C33S -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.57' -and
  [string]$ticket.candidate.versionCode -ceq '2026081357' -and
...
$uploadRunbookPath = Resolve-C33SFile `
  -Path 'docs/quality/UAW-C33S-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33S pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33SSanitizedText -Text $uploadRunbook -Label 'C33S upload runbook'
Assert-C33S -Condition (
...
$resolvedStatePath = Resolve-C33SFile -Path $StatePath -Label 'C33S state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33SSanitizedText -Text $stateRaw -Label 'C33S state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33SFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33S aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33SSanitizedText -Text $aggregateRaw -Label 'C33S aggregate'
...
Assert-C33S -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.57' -and
  [string]$state.candidate.versionCode -ceq '2026081357' -and
...
Assert-C33S -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33S-R60-57-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.57' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081357'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33SFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33S pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33SFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33S end-to-end founder authorization'
Assert-C33S -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33S-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33S-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33S-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33S -Condition (
  $historicalContracts.Count -eq 8 -and
  $aggregateHistoricalContracts.Count -eq 8
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33SFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33SFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33SFile `
...
$rejectedC33OStatePath = Resolve-C33SFile `
...
$rejectedC33PStatePath = Resolve-C33SFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33SFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33SFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33SFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33SFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33SFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33SFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33S founder launcher'
$wrapperPath = Resolve-C33SFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33SFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33S interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33SFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33SPowerShellOwner -Path $launcherPath -Label 'C33S founder launcher'
Assert-C33SPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33SPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33S interrupted-postbuild recovery owner'
Assert-C33SPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33s-r60-57-authentication-no-regression-release-readiness.ps1'
...
Assert-C33S -Condition (
  ...
  $launcher.IndexOf('C33S r60.57 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33S founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33S binding, gate order or single appbundle owner changed.'
Assert-C33S -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33SFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33SFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33SFile `
...
$googleStatePath = Resolve-C33SFile `
...
Assert-C33SSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33SFile `
...
Assert-C33SSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33SManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33SFile `
...
Write-Output (
  'C33S r60.57 no-regression release gate passed: ' +
