...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c34g.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34G postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34GRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34G existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34GRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34GRecovery -Condition (
...
function Write-C34GRecoveryState {
  param(
  ...
  Assert-C34GRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$candidateId =
  'UAW-C34G-R60-71-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRoot =
  'artifacts/quality/uaw-c34g-r60-71-authentication-no-regression-preparation-20260817-01'
$stateFile = Resolve-C34GRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34GRecoveryPath `
...
$transientGoogleServices = Resolve-C34GRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34GRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.71-2026081371-release.aab"
$sealedPath = Resolve-C34GRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34GRecoveryPath `
...
Assert-C34GRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34GRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.71' -and
  [string]$provenance.versionCode -ceq '2026081371' -and
...
$buildLog = Resolve-C34GRecoveryPath `
...
Assert-C34GRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34GRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34g-recovery-state-write'
  Write-C34GRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34g-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c34g-r60-71-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C34G existing-AAB postbuild lifecycle recovery passed: ' +
