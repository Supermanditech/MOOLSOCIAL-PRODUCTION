...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33t.json',
...
function Assert-C33T {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33T r60.58 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33TFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33T -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33TSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33T -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33TPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33T -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33TManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33T -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33TFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33T -Condition (
...
$ticketId = 'UAW-C33T-R60-58-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33TFile `
  -Path 'config/uaw-c33t-r60-58-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33T ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33TSanitizedText -Text $ticketRaw -Label 'C33T ticket'
Assert-C33T -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '89991E90D652154AD1B263D5218755C7C9325E60F0B6B198AF6461A12D58CCC9'
...
Assert-C33T -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.58' -and
  [string]$ticket.candidate.versionCode -ceq '2026081358' -and
...
$uploadRunbookPath = Resolve-C33TFile `
  -Path 'docs/quality/UAW-C33T-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33T pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33TSanitizedText -Text $uploadRunbook -Label 'C33T upload runbook'
Assert-C33T -Condition (
...
$resolvedStatePath = Resolve-C33TFile -Path $StatePath -Label 'C33T state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33TSanitizedText -Text $stateRaw -Label 'C33T state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33TFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33T aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33TSanitizedText -Text $aggregateRaw -Label 'C33T aggregate'
...
Assert-C33T -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.58' -and
  [string]$state.candidate.versionCode -ceq '2026081358' -and
...
Assert-C33T -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33T-R60-58-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.58' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081358'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33TFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33T pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33TFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33T end-to-end founder authorization'
Assert-C33T -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33T-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33T-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33T-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33T -Condition (
  $historicalContracts.Count -eq 9 -and
  $aggregateHistoricalContracts.Count -eq 9
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33TFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33TFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33TFile `
...
$rejectedC33OStatePath = Resolve-C33TFile `
...
$rejectedC33PStatePath = Resolve-C33TFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33TFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33TFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C33TFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33TFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33TFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33TFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33TFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33T founder launcher'
$wrapperPath = Resolve-C33TFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33TFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33T interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33TFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33TPowerShellOwner -Path $launcherPath -Label 'C33T founder launcher'
Assert-C33TPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33TPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33T interrupted-postbuild recovery owner'
Assert-C33TPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33t-r60-58-authentication-no-regression-release-readiness.ps1'
...
Assert-C33T -Condition (
  ...
  $launcher.IndexOf('C33T r60.58 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33T founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33T binding, gate order or single appbundle owner changed.'
Assert-C33T -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33TFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33TFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33TFile `
...
$googleStatePath = Resolve-C33TFile `
...
Assert-C33TSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33TFile `
...
Assert-C33TSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33TManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33TFile `
...
Write-Output (
  'C33T r60.58 no-regression release gate passed: ' +
