...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33u.json',
...
function Assert-C33U {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33U r60.59 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33UFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33U -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33USanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33U -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33UPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33U -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33UManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33U -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33UFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33U -Condition (
...
$ticketId = 'UAW-C33U-R60-59-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33UFile `
  -Path 'config/uaw-c33u-r60-59-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33U ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33USanitizedText -Text $ticketRaw -Label 'C33U ticket'
Assert-C33U -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '786BD5DA8F9839A3CC635A7C7C895AFDA174346B2E3799A351EAA7B542A310E5'
...
Assert-C33U -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.59' -and
  [string]$ticket.candidate.versionCode -ceq '2026081359' -and
...
$uploadRunbookPath = Resolve-C33UFile `
  -Path 'docs/quality/UAW-C33U-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33U pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33USanitizedText -Text $uploadRunbook -Label 'C33U upload runbook'
Assert-C33U -Condition (
...
$resolvedStatePath = Resolve-C33UFile -Path $StatePath -Label 'C33U state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33USanitizedText -Text $stateRaw -Label 'C33U state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33UFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33U aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33USanitizedText -Text $aggregateRaw -Label 'C33U aggregate'
...
Assert-C33U -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.59' -and
  [string]$state.candidate.versionCode -ceq '2026081359' -and
...
Assert-C33U -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33U-R60-59-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.59' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081359'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33UFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33U pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33UFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33U end-to-end founder authorization'
Assert-C33U -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33U-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33U-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33U-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33U -Condition (
  $historicalContracts.Count -eq 10 -and
  $aggregateHistoricalContracts.Count -eq 10
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33UFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33UFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33UFile `
...
$rejectedC33OStatePath = Resolve-C33UFile `
...
$rejectedC33PStatePath = Resolve-C33UFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33UFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C33UFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C33UFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C33UFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33UFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33UFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33UFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33UFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33U founder launcher'
$wrapperPath = Resolve-C33UFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33UFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33U interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33UFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33UPowerShellOwner -Path $launcherPath -Label 'C33U founder launcher'
Assert-C33UPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33UPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33U interrupted-postbuild recovery owner'
Assert-C33UPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33u-r60-59-authentication-no-regression-release-readiness.ps1'
...
Assert-C33U -Condition (
  ...
  $launcher.IndexOf('C33U r60.59 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C33U founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33U binding, gate order or single appbundle owner changed.'
Assert-C33U -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33UFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33UFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33UFile `
...
$googleStatePath = Resolve-C33UFile `
...
Assert-C33USanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33UFile `
...
Assert-C33USanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33UManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33UFile `
...
Write-Output (
  'C33U r60.59 no-regression release gate passed: ' +
