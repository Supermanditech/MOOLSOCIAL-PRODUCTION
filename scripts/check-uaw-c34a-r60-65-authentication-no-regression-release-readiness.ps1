...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c34a.json',
...
function Assert-C34A {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34A r60.65 no-regression release gate rejected: $Message"
  }
}

function Resolve-C34AFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C34A -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C34ASanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C34A -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C34APowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C34A -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C34AManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C34A -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C34AFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C34A -Condition (
...
$ticketPath = Resolve-C34AFile `
  -Path 'config/uaw-c34a-r60-65-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C34A ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C34ASanitizedText -Text $ticketRaw -Label 'C34A ticket'
Assert-C34A -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    'D641D6A989B432C5C1904A3ACECDF2C8927CDE2CD31919CC25B8446FEF188F50'
...
Assert-C34A -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.65' -and
  [string]$ticket.candidate.versionCode -ceq '2026081365' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C33Z-R60-64-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
...
$uploadRunbookPath = Resolve-C34AFile `
  -Path 'docs/quality/UAW-C34A-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C34A pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C34ASanitizedText -Text $uploadRunbook -Label 'C34A upload runbook'
Assert-C34A -Condition (
...
$resolvedStatePath = Resolve-C34AFile -Path $StatePath -Label 'C34A state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C34ASanitizedText -Text $stateRaw -Label 'C34A state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C34AFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C34A aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C34ASanitizedText -Text $aggregateRaw -Label 'C34A aggregate'
...
Assert-C34A -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.65' -and
  [string]$state.candidate.versionCode -ceq '2026081365' -and
...
Assert-C34A -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34A-R60-65-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.65' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081365'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C34AFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C34A pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C34AFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C34A end-to-end founder authorization'
Assert-C34A -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C34A-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C34A-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C34A-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C34A -Condition (
  $historicalContracts.Count -eq 16 -and
  $aggregateHistoricalContracts.Count -eq 16
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C34AFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C34AFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C34AFile `
...
$rejectedC33OStatePath = Resolve-C34AFile `
...
$rejectedC33PStatePath = Resolve-C34AFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C34AFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C34AFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C34AFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C34AFile `
...
$rejectedC33UStatePath = Resolve-C34AFile `
...
$rejectedC33VStatePath = Resolve-C34AFile `
...
$rejectedC33WStatePath = Resolve-C34AFile `
...
$rejectedC33XStatePath = Resolve-C34AFile `
...
$rejectedC33YStatePath = Resolve-C34AFile `
...
$rejectedC33ZStatePath = Resolve-C34AFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C34AFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C34AFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C34AFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C34AFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C34A founder launcher'
$wrapperPath = Resolve-C34AFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C34AFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C34A interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C34AFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C34A predeclared source-cycle owner'
$resultRetentionPath = Resolve-C34AFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C34APowerShellOwner -Path $launcherPath -Label 'C34A founder launcher'
Assert-C34APowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C34APowerShellOwner `
  -Path $recoveryPath `
  -Label 'C34A interrupted-postbuild recovery owner'
Assert-C34APowerShellOwner `
  -Path $cycleOwnerPath `
...
$gateNeedle = 'scripts/check-uaw-c34a-r60-65-authentication-no-regression-release-readiness.ps1'
...
Assert-C34A -Condition (
  ...
  $launcher.IndexOf('C34A r60.65 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C34A founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C34A binding, gate order or single appbundle owner changed.'
Assert-C34A -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C34AFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C34AFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C34AFile `
...
$googleStatePath = Resolve-C34AFile `
...
Assert-C34ASanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C34AFile `
...
Assert-C34ASanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C34AFile `
...
Write-Output (
  'C34A r60.65 no-regression release gate passed: ' +
