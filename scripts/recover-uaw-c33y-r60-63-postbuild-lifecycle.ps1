...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33y.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33Y postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33YRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33Y existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33YRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33YRecovery -Condition (
...
function Write-C33YRecoveryState {
  param(
  ...
  Assert-C33YRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33y-r60-63-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33YRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33YRecoveryPath `
...
$transientGoogleServices = Resolve-C33YRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33YRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.63-2026081363-release.aab"
$sealedPath = Resolve-C33YRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33YRecoveryPath `
...
Assert-C33YRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33YRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.63' -and
  [string]$provenance.versionCode -ceq '2026081363' -and
...
$buildLog = Resolve-C33YRecoveryPath `
...
Assert-C33YRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33YRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33y-recovery-state-write'
  Write-C33YRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33y-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33y-r60-63-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33Y existing-AAB postbuild lifecycle recovery passed: ' +
