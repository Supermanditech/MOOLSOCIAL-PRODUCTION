...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c34d.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34D postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34DRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34D existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34DRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34DRecovery -Condition (
...
function Write-C34DRecoveryState {
  param(
  ...
  Assert-C34DRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c34d-r60-68-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C34DRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34DRecoveryPath `
...
$transientGoogleServices = Resolve-C34DRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34DRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.68-2026081368-release.aab"
$sealedPath = Resolve-C34DRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34DRecoveryPath `
...
Assert-C34DRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34DRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.68' -and
  [string]$provenance.versionCode -ceq '2026081368' -and
...
$buildLog = Resolve-C34DRecoveryPath `
...
Assert-C34DRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34DRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34d-recovery-state-write'
  Write-C34DRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34d-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c34d-r60-68-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C34D existing-AAB postbuild lifecycle recovery passed: ' +
