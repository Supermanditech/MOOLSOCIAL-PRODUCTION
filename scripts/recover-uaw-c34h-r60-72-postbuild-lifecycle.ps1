...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c34h.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34H postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34HRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34H existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34HRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34HRecovery -Condition (
...
function Write-C34HRecoveryState {
  param(
  ...
  Assert-C34HRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$candidateId =
  'UAW-C34H-R60-72-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRoot =
  'artifacts/quality/uaw-c34h-r60-72-authentication-no-regression-preparation-20260817-01'
$stateFile = Resolve-C34HRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34HRecoveryPath `
...
$transientGoogleServices = Resolve-C34HRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34HRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.72-2026081372-release.aab"
$sealedPath = Resolve-C34HRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34HRecoveryPath `
...
Assert-C34HRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34HRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.72' -and
  [string]$provenance.versionCode -ceq '2026081372' -and
...
$buildLog = Resolve-C34HRecoveryPath `
...
Assert-C34HRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34HRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34h-recovery-state-write'
  Write-C34HRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34h-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c34h-r60-72-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C34H existing-AAB postbuild lifecycle recovery passed: ' +
