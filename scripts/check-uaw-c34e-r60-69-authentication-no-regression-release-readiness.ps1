...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c34e.json',
...
function Assert-C34E {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34E r60.69 no-regression release gate rejected: $Message"
  }
}

function Resolve-C34EFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C34E -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C34ESanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C34E -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C34EPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C34E -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C34EManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C34E -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C34EFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C34E -Condition (
...
$ticketId = 'UAW-C34E-R60-69-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C34EFile `
  -Path 'config/uaw-c34e-r60-69-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C34E ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C34ESanitizedText -Text $ticketRaw -Label 'C34E ticket'
Assert-C34E -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '4635E40247B0E6B20C4C924E6210E7256910FBEFC355E8979C259EF32C1BA905'
...
Assert-C34E -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.69' -and
  [string]$ticket.candidate.versionCode -ceq '2026081369' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C34D-R60-68-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  ...
  [string]$ticket.classificationReason -match 'REG2662' -and
  [string]$ticket.classificationReason -match 'REG2663' -and
...
$uploadRunbookPath = Resolve-C34EFile `
  -Path 'docs/quality/UAW-C34E-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C34E pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C34ESanitizedText -Text $uploadRunbook -Label 'C34E upload runbook'
Assert-C34E -Condition (
...
$resolvedStatePath = Resolve-C34EFile -Path $StatePath -Label 'C34E state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C34ESanitizedText -Text $stateRaw -Label 'C34E state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C34EFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C34E aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C34ESanitizedText -Text $aggregateRaw -Label 'C34E aggregate'
...
Assert-C34E -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.69' -and
  [string]$state.candidate.versionCode -ceq '2026081369' -and
...
Assert-C34E -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34E-R60-69-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.69' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081369'
...
$browserQualificationPath = Resolve-C34EFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C34E pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C34EFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C34E end-to-end founder authorization'
Assert-C34E -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C34E-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C34E-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C34E-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C34E -Condition (
  $historicalContracts.Count -eq 20 -and
  $aggregateHistoricalContracts.Count -eq 20
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C34EFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C34EFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C34EFile `
...
$rejectedC33OStatePath = Resolve-C34EFile `
...
$rejectedC33PStatePath = Resolve-C34EFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C34EFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C34EFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C34EFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C34EFile `
...
$rejectedC33UStatePath = Resolve-C34EFile `
...
$rejectedC33VStatePath = Resolve-C34EFile `
...
$rejectedC33WStatePath = Resolve-C34EFile `
...
$rejectedC33XStatePath = Resolve-C34EFile `
...
$rejectedC33YStatePath = Resolve-C34EFile `
...
$rejectedC33ZStatePath = Resolve-C34EFile `
...
$rejectedC34AStatePath = Resolve-C34EFile `
...
$rejectedC34BStatePath = Resolve-C34EFile `
...
$rejectedC34CStatePath = Resolve-C34EFile `
...
$rejectedC34DStatePath = Resolve-C34EFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C34EFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C34EFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C34EFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C34EFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C34E founder launcher'
$wrapperPath = Resolve-C34EFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C34EFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C34E interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C34EFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C34E predeclared source-cycle owner'
$resultRetentionPath = Resolve-C34EFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C34EPowerShellOwner -Path $launcherPath -Label 'C34E founder launcher'
Assert-C34EPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C34EPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C34E interrupted-postbuild recovery owner'
Assert-C34EPowerShellOwner `
  -Path $cycleOwnerPath `
...
Assert-C34E -Condition (
  ...
  $cycleOwner.IndexOf(
    '$manifestRelative = "$evidenceRelative/source-manifest-c34e-registry-2634.txt"',
    [StringComparison]::Ordinal
  ) -ge 0 -and
  $cycleOwner.IndexOf('C34E source-cycle prerequisite preflight passed:', [StringComparison]::Ordinal) -ge 0 -and
...
$gateNeedle = 'scripts/check-uaw-c34e-r60-69-authentication-no-regression-release-readiness.ps1'
...
Assert-C34E -Condition (
  ...
  $launcher.IndexOf('C34E r60.69 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C34E founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C34E binding, gate order or single appbundle owner changed.'
Assert-C34E -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C34EFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C34EFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C34EFile `
...
$googleStatePath = Resolve-C34EFile `
...
Assert-C34ESanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C34EFile `
...
Assert-C34ESanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C34EFile `
...
Write-Output (
  'C34E r60.69 no-regression release gate passed: ' +
