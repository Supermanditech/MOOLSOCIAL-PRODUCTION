...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33s.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33S postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33SRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33S existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33SRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33SRecovery -Condition (
...
function Write-C33SRecoveryState {
  param(
  ...
  Assert-C33SRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33s-r60-57-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33SRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33SRecoveryPath `
...
$transientGoogleServices = Resolve-C33SRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33SRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.57-2026081357-release.aab"
$sealedPath = Resolve-C33SRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33SRecoveryPath `
...
Assert-C33SRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33SRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.57' -and
  [string]$provenance.versionCode -ceq '2026081357' -and
...
$buildLog = Resolve-C33SRecoveryPath `
...
Assert-C33SRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33SRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33s-recovery-state-write'
  Write-C33SRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33s-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33s-r60-57-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33S existing-AAB postbuild lifecycle recovery passed: ' +
