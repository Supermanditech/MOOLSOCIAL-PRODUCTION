...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34F source cycle requires PowerShell 7 or newer.'
}
...
if ($branch -cne 'remediation/prototype-conformance-2026-07-20' -or $head -cne 'f6dfe7587aa02d782e94282d14af8bafff48ded0') {
  throw 'C34F source cycle repository identity changed.'
}

$ticketId = 'UAW-C34F-R60-70-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRelative = 'artifacts/quality/uaw-c34f-r60-70-authentication-no-regression-preparation-20260817-01'
$evidenceRoot = Join-Path $root $evidenceRelative
$manifestRelative = "$evidenceRelative/source-manifest-c34f-registry-2646.txt"
$focusedRelative = "$evidenceRelative/focused-test-manifest-c34f.txt"
$stateRelative = 'config/successor-aab-regression-hard-gate-state-c34f.json'
$statePath = Join-Path $root $stateRelative
$prefix = 'c34f-cycle-{0:d2}' -f $Cycle
$logs = [ordered]@{
...
foreach ($path in $logs.Values) {
  if (Test-Path -LiteralPath $path) { throw 'C34F source-cycle evidence already exists; overwrite is forbidden.' }
}
foreach ($path in @($statePath, (Join-Path $root $manifestRelative), (Join-Path $root $focusedRelative))) {
  if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw 'C34F source-cycle prerequisite is missing.' }
}
if ($PrerequisiteOnly) {
  Write-Output 'C34F source-cycle prerequisite preflight passed: exact state, registry-2646 manifest and focused manifest paths exist; testsStarted=false.'
  return
}
...
) { throw 'C34F source-cycle held-authority state changed.' }

function ConvertTo-C34FLines {
  param([object[]]$Values)
  return @($Values | ForEach-Object { if ($null -eq $_) { '' } else { [string]$_ } })
}

function Assert-C34FSanitized {
  param([string[]]$Lines)
  $text = $Lines -join "`n"
  foreach ($pattern in @(
    ...
    if ([regex]::IsMatch($text, $pattern)) { throw 'C34F source-cycle output violated the sanitized evidence boundary.' }
  }
}

function Write-C34FLog {
  param([string]$Path, [string[]]$Lines)
  Assert-C34FSanitized -Lines $Lines
  [IO.File]::WriteAllLines($Path, $Lines, [Text.UTF8Encoding]::new($false))
}

function Invoke-C34FNative {
  ...
  $lines = ConvertTo-C34FLines -Values $raw
  Write-C34FLog -Path $LogPath -Lines $lines
  if ($code -ne 0) { throw 'C34F native source-cycle step failed; inspect only the sanitized retained log.' }
  return $lines
}

$staticLines = [Collections.Generic.List[string]]::new()
function Add-C34FStatic {
  param([object[]]$Values)
  foreach ($line in (ConvertTo-C34FLines -Values $Values)) { $staticLines.Add($line) }
}

$manifestOwner = Join-Path $root 'scripts/new-c30v-source-manifest.ps1'
Add-C34FStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Add-C34FStatic -Values @(& (Join-Path $root 'scripts/check-codex-development-regression-memory.ps1') -Phase implementation)
Add-C34FStatic -Values @(& (Join-Path $root 'scripts/check-mvp-scope-gate-state.ps1') -StatePath 'config/mvp-scope-gate-state.json' -CandidateId $ticketId -RequireExecutionAuthorized -RepositoryRoot $root)
Add-C34FStatic -Values @(& (Join-Path $root 'scripts/check-approved-ui-locks.ps1') -RepositoryRoot $root)

$candidateGate = Join-Path $root 'scripts/check-uaw-c34f-r60-70-authentication-no-regression-release-readiness.ps1'
Push-Location 'C:\WINDOWS\system32'
try {
  Add-C34FStatic -Values @(& $candidateGate -Phase source -StatePath $stateRelative -RepositoryRoot $root)
} finally { Pop-Location }
...
Add-C34FStatic -Values $winRaw
if ($winCode -ne 0) { throw 'C34F Windows PowerShell source gate failed.' }

$pwsh = (Get-Command pwsh -ErrorAction Stop).Source
$flutterRunner = Join-Path $root 'tmp/run-c30t-authoritative-flutter-manifest-audit.ps1'
$flutterLines = Invoke-C34FNative -File $pwsh -Arguments @(
...
$analyzerLines = Invoke-C34FNative -File $flutter -Arguments @('analyze', '--no-pub') `
  -WorkingDirectory (Join-Path $root 'apps/mobile') -LogPath $logs.analyzer
$npm = (Get-Command npm.cmd -ErrorAction Stop).Source
$backendTypecheckLines = Invoke-C34FNative -File $npm -Arguments @('run', 'typecheck') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTypecheck
$backendTestLines = Invoke-C34FNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'backend/functions') -LogPath $logs.backendTest
$webTestLines = Invoke-C34FNative -File $npm -Arguments @('test') `
  -WorkingDirectory (Join-Path $root 'apps/web') -LogPath $logs.webTest

Add-C34FStatic -Values @(& $manifestOwner -ComparePath $manifestRelative -RepositoryRoot $root)
Write-C34FLog -Path $logs.static -Lines $staticLines.ToArray()
...
if (-not $flutterMatch.Success) { throw 'C34F Flutter summary marker is missing.' }
...
) { throw 'C34F source-cycle bounded result counts changed.' }

$summary = [ordered]@{
  schemaVersion = 1
  ticketId = $ticketId
  cycle = $Cycle
  registryEntryCount = 2646
  registrySha256 = '5DB74D0BDBEAAB4B9A57D414CA56C90906E368033AF46E429010ADAE7B743C9B'
...
Assert-C34FSanitized -Lines @($summaryText)
[IO.File]::WriteAllText($logs.summary, $summaryText, [Text.UTF8Encoding]::new($false))
Write-Output "C34F source cycle passed: cycle=$Cycle; Flutter=501/3/0; backend=537; web=8; sourceUnchanged=true; PlayWrites=0."
