...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c33r.json',
...
function Assert-C33R {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33R r60.56 no-regression release gate rejected: $Message"
  }
}

function Resolve-C33RFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C33R -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C33RSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C33R -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C33RPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C33R -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C33RManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C33R -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C33RFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C33R -Condition (
...
$ticketId = 'UAW-C33R-R60-56-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C33RFile `
  -Path 'config/uaw-c33r-r60-56-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C33R ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C33RSanitizedText -Text $ticketRaw -Label 'C33R ticket'
Assert-C33R -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '89198D667EABB302FD42BEA7672107E1B881A9B898C0A4DDE1DCB4F46E6F07D6'
...
Assert-C33R -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.56' -and
  [string]$ticket.candidate.versionCode -ceq '2026081356' -and
...
$uploadRunbookPath = Resolve-C33RFile `
  -Path 'docs/quality/UAW-C33R-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C33R pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C33RSanitizedText -Text $uploadRunbook -Label 'C33R upload runbook'
Assert-C33R -Condition (
...
$resolvedStatePath = Resolve-C33RFile -Path $StatePath -Label 'C33R state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C33RSanitizedText -Text $stateRaw -Label 'C33R state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C33RFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C33R aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C33RSanitizedText -Text $aggregateRaw -Label 'C33R aggregate'
...
Assert-C33R -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.56' -and
  [string]$state.candidate.versionCode -ceq '2026081356' -and
...
Assert-C33R -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C33R-R60-56-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.56' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081356'
) -Message 'aggregate identity or candidate changed.'

$browserQualificationPath = Resolve-C33RFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C33R pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C33RFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C33R end-to-end founder authorization'
Assert-C33R -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C33R-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C33R-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C33R-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C33R -Condition (
  $historicalContracts.Count -eq 7 -and
  $aggregateHistoricalContracts.Count -eq 7
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C33RFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C33RFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C33RFile `
...
$rejectedC33OStatePath = Resolve-C33RFile `
...
$rejectedC33PStatePath = Resolve-C33RFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C33RFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C33RFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C33RFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C33RFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C33RFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C33R founder launcher'
$wrapperPath = Resolve-C33RFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C33RFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C33R interrupted-postbuild recovery owner'
$resultRetentionPath = Resolve-C33RFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C33RPowerShellOwner -Path $launcherPath -Label 'C33R founder launcher'
Assert-C33RPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C33RPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C33R interrupted-postbuild recovery owner'
Assert-C33RPowerShellOwner `
...
$gateNeedle = 'scripts/check-uaw-c33r-r60-56-authentication-no-regression-release-readiness.ps1'
...
Assert-C33R -Condition (
  ...
  $launcher.IndexOf(
    "throw 'C33R founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C33R binding, gate order or single appbundle owner changed.'
Assert-C33R -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C33RFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C33RFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C33RFile `
...
$googleStatePath = Resolve-C33RFile `
...
Assert-C33RSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C33RFile `
...
Assert-C33RSanitizedText -Text $registryRaw -Label 'regression registry'
...
if ($cycles -eq 0) {
  ...
  Assert-C33RManifestCurrent -ManifestPath $manifestPath
  $focusedManifestPath = Resolve-C33RFile `
...
Write-Output (
  'C33R r60.56 no-regression release gate passed: ' +
