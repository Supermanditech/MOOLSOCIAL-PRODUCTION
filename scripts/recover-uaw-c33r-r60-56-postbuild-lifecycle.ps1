...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33r.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33R postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33RRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33R existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33RRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33RRecovery -Condition (
...
function Write-C33RRecoveryState {
  param(
  ...
  Assert-C33RRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33r-r60-56-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33RRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33RRecoveryPath `
...
$transientGoogleServices = Resolve-C33RRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33RRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.56-2026081356-release.aab"
$sealedPath = Resolve-C33RRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33RRecoveryPath `
...
Assert-C33RRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33RRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.56' -and
  [string]$provenance.versionCode -ceq '2026081356' -and
...
$buildLog = Resolve-C33RRecoveryPath `
...
Assert-C33RRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33RRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33r-recovery-state-write'
  Write-C33RRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33r-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33r-r60-56-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33R existing-AAB postbuild lifecycle recovery passed: ' +
