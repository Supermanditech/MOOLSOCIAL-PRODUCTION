...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33n.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33N postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33NRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33N existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33NRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33NRecovery -Condition (
...
function Write-C33NRecoveryState {
  param(
  ...
  Assert-C33NRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33n-r60-52-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33NRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33NRecoveryPath `
...
$transientGoogleServices = Resolve-C33NRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33NRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.52-2026081352-release.aab"
$sealedPath = Resolve-C33NRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33NRecoveryPath `
...
Assert-C33NRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33NRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.52' -and
  [string]$provenance.versionCode -ceq '2026081352' -and
...
$buildLog = Resolve-C33NRecoveryPath `
...
Assert-C33NRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33NRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33n-recovery-state-write'
  Write-C33NRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33n-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33n-r60-52-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33N existing-AAB postbuild lifecycle recovery passed: ' +
