...
param(
  [string]$StatePath,
  [string]$CandidateId = 'UAW-C30U-POST-R60-45-SOCIAL-REPAIRS-PLAY-INTERNAL-ACCEPTANCE',
...
function Assert-C30U {
  param([Parameter(Mandatory)][bool]$Condition, [Parameter(Mandatory)][string]$Message)
  if (-not $Condition) { throw "C30U Play Internal AAB gate rejected: $Message" }
}

function Resolve-RepoFile {
  param([Parameter(Mandatory)][string]$Path, [Parameter(Mandatory)][string]$Label)
  Assert-C30U -Condition (-not [string]::IsNullOrWhiteSpace($Path)) -Message "$Label path is missing."
  Assert-C30U -Condition (-not [IO.Path]::IsPathRooted($Path)) -Message "$Label must be repository-relative."
  $resolved = [IO.Path]::GetFullPath((Join-Path $root $Path))
  Assert-C30U -Condition ($resolved.StartsWith($prefix, [StringComparison]::OrdinalIgnoreCase)) -Message "$Label escaped the repository."
  Assert-C30U -Condition (Test-Path -LiteralPath $resolved -PathType Leaf) -Message "$Label is missing."
  return $resolved
}
...
function Assert-SourceManifestCurrent {
  ...
  foreach ($line in Get-Content -LiteralPath $Path) {
    ...
    Assert-C30U -Condition $row.Success -Message 'source manifest row is malformed.'
    $owner = Resolve-RepoFile -Path $row.Groups[2].Value -Label 'source owner'
    Assert-C30U -Condition (
...
Assert-C30U -Condition ($stateFile.StartsWith($prefix, [StringComparison]::OrdinalIgnoreCase)) -Message 'state escaped the repository.'
Assert-C30U -Condition (Test-Path -LiteralPath $stateFile -PathType Leaf) -Message 'state is missing.'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
Assert-C30U -Condition (
  [int]$state.schemaVersion -eq 1 -and
  [string]$state.contractId -ceq 'PLAY-INTERNAL-AAB-REGRESSION-GATES-C30U-001'
) -Message 'state contract changed.'
Assert-C30U -Condition ([string]$state.candidate.id -ceq $CandidateId) -Message 'candidate changed.'
Assert-C30U -Condition (
  [string]$state.candidate.versionName -ceq '1.0.0-r60.46' -and
  [string]$state.candidate.versionCode -ceq '2026081346' -and
...
) -Message 'candidate is not exact r60.46.'

$branch = (& git -C $root branch --show-current).Trim()
Assert-C30U -Condition ($LASTEXITCODE -eq 0 -and $branch -ceq [string]$state.candidate.branch) -Message 'branch changed.'
$head = (& git -C $root rev-parse HEAD).Trim()
Assert-C30U -Condition ($LASTEXITCODE -eq 0 -and $head -ceq [string]$state.candidate.head) -Message 'HEAD changed.'
...
Assert-C30U -Condition ($ticketHash -ceq [string]$state.ticketSha256) -Message 'sealed ticket hash changed.'
...
$qualifierPath = Resolve-RepoFile -Path 'scripts/qualify-play-internal-social-repairs-c30u.ps1' -Label 'C30U qualifier'
...
) -Message 'C30U qualifier does not preserve the OPPO-supported exact Google Play installer gate.'

Assert-C30U -Condition (
  [bool]$ticket.authority.DevMoolSocialContentDeploymentAuthorized -and
...
Assert-C30U -Condition (
  [string]$aggregate.contractId -ceq 'PLAY-INTERNAL-SOCIAL-REPAIRS-ACCEPTANCE-GATES-C30U-001' -and
  [string]$aggregate.candidate.id -ceq $CandidateId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.46' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081346' -and
...
$predecessorAabPath = Resolve-RepoFile -Path ([string]$aggregate.predecessor.aabStatePath) -Label 'C30T AAB predecessor'
$predecessorAggregatePath = Resolve-RepoFile -Path ([string]$aggregate.predecessor.aggregateStatePath) -Label 'C30T aggregate predecessor'
Assert-C30U -Condition (
...
Assert-C30U -Condition (
  ...
  [int]$predecessor.playReleaseResult.uploadCount -eq 1 -and
  [int]$predecessor.installResult.candidateInstallCount -eq 1 -and
  [string]$predecessor.installResult.installerPackageName -ceq 'com.android.vending' -and
  [string]$predecessor.installResult.installedVersionCode -ceq '2026081345' -and
  [string]$predecessor.installResult.installedVersionName -ceq '1.0.0-r60.45'
) -Message 'consumed Play predecessor identity changed.'
...
Assert-C30U -Condition (
  ...
  [bool]$aggregate.providerAndHostingBoundary.deploymentRequired -and
...
if ([string]$state.sourceQualification.state -ceq 'passed_two_identical_cycles_after_Dev_content_deployment') {
...
switch ($Phase) {
  'build' {
    ...
    ) -Message 'single build is not ready or final source/deployment proof differs.'
  }
  'postbuild' {
    Assert-C30U -Condition (
  ...
  'postupload' {
    Assert-C30U -Condition (
      ...
      [string]$state.playReleaseResult.versionCode -ceq '2026081346' -and
      [string]$state.playReleaseResult.versionName -ceq '1.0.0-r60.46'
  ...
  'postinstall' {
    Assert-C30U -Condition (
      ...
      [string]$state.installResult.installedVersionCode -ceq '2026081346' -and
      [string]$state.installResult.installedVersionName -ceq '1.0.0-r60.46' -and
  ...
  'journey' {
    Assert-C30U -Condition (
      [string]$state.machineState -ceq 'Play_installed_identity_sealed_journeys_pending' -and
      [string]$state.installResult.installedVersionCode -ceq '2026081346'
...
Write-Output "C30U Play Internal AAB gate passed: phase=$Phase; buildCount=$($state.buildResult.buildCount); uploadCount=$($state.playReleaseResult.uploadCount); installCount=$($state.installResult.candidateInstallCount)."
