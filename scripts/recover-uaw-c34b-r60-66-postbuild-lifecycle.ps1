...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34B postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34BRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34B existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34BRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34BRecovery -Condition (
...
function Write-C34BRecoveryState {
  param(
  ...
  Assert-C34BRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c34b-r60-66-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C34BRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34BRecoveryPath `
...
$transientGoogleServices = Resolve-C34BRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34BRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.66-2026081366-release.aab"
$sealedPath = Resolve-C34BRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34BRecoveryPath `
...
Assert-C34BRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34BRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.66' -and
  [string]$provenance.versionCode -ceq '2026081366' -and
...
$buildLog = Resolve-C34BRecoveryPath `
...
Assert-C34BRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34BRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34b-recovery-state-write'
  Write-C34BRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34b-recovery-aggregate-write'
  & (Join-Path $root `
...
Write-Output (
  'C34B existing-AAB postbuild lifecycle recovery passed: ' +
