...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c34g.json',
...
function Assert-C34G {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34G r60.71 no-regression release gate rejected: $Message"
  }
}
...
function Resolve-C34GFile {
  ...
  Assert-C34G -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C34GSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C34G -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C34GPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C34G -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C34GManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C34G -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C34GFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C34G -Condition (
...
Assert-C34GSanitizedText -Text $ticketRaw -Label 'C34G ticket'
Assert-C34G -Condition (
  (Get-FileHash -Algorithm SHA256 -LiteralPath $ticketPath).Hash -ceq
    '9C0AF2BE02014179F9D6D2A85D397633D85BB65E933292F6727C324AECB84669'
...
Assert-C34G -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.71' -and
  [string]$ticket.candidate.versionCode -ceq '2026081371' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C34F-R60-70-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  [string]$ticket.classificationReason -match 'REG2676' -and
  [string]$ticket.classificationReason -match 'outer-loop label' -and
  ...
  @($ticket.minimumCompleteScope) -contains
    'repair_only_the_existing_authoritative_Flutter_audit_outer_loop_control_so_non_object_classifications_never_reach_event_access' -and
  @($ticket.minimumCompleteScope) -contains
    'add_one_focused_dual_host_executable_prevention_for_blank_non_JSON_JSON_null_typed_and_untyped_event_lines' -and
...
$uploadRunbookPath = Resolve-C34GFile `
  -Path 'docs/quality/UAW-C34G-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260817.md' `
  -Label 'C34G pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C34GSanitizedText -Text $uploadRunbook -Label 'C34G upload runbook'
Assert-C34G -Condition (
...
$resolvedStatePath = Resolve-C34GFile -Path $StatePath -Label 'C34G state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C34GSanitizedText -Text $stateRaw -Label 'C34G state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C34GFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C34G aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C34GSanitizedText -Text $aggregateRaw -Label 'C34G aggregate'
...
Assert-C34G -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.71' -and
  [string]$state.candidate.versionCode -ceq '2026081371' -and
...
Assert-C34G -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34G-R60-71-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.71' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081371'
...
$founderAuthorizationPath = Resolve-C34GFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C34G end-to-end founder authorization'
Assert-C34G -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C34G-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260817.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C34G-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260817.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C34G-END-TO-END-FOUNDER-AUTHORIZATION-20260817.md' -and
...
Assert-C34G -Condition (
  $historicalContracts.Count -eq 22 -and
  $aggregateHistoricalContracts.Count -eq 22
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C34GFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C34GFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C34GFile `
...
$rejectedC33OStatePath = Resolve-C34GFile `
...
$rejectedC33PStatePath = Resolve-C34GFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C34GFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C34GFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C34GFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C34GFile `
...
$rejectedC33UStatePath = Resolve-C34GFile `
...
$rejectedC33VStatePath = Resolve-C34GFile `
...
$rejectedC33WStatePath = Resolve-C34GFile `
...
$rejectedC33XStatePath = Resolve-C34GFile `
...
$rejectedC33YStatePath = Resolve-C34GFile `
...
$rejectedC33ZStatePath = Resolve-C34GFile `
...
$rejectedC34AStatePath = Resolve-C34GFile `
...
$rejectedC34BStatePath = Resolve-C34GFile `
...
$rejectedC34CStatePath = Resolve-C34GFile `
...
$rejectedC34DStatePath = Resolve-C34GFile `
...
$rejectedC34EStatePath = Resolve-C34GFile `
...
$rejectedC34FStatePath = Resolve-C34GFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C34GFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C34GFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C34GFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C34GFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C34G founder launcher'
$wrapperPath = Resolve-C34GFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C34GFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C34G interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C34GFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C34G predeclared source-cycle owner'
$resultRetentionPath = Resolve-C34GFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
$loopControlGatePath = Resolve-C34GFile `
  -Path 'scripts/check-uaw-c34g-authoritative-flutter-nonobject-loop-control.ps1' `
  -Label 'C34G authoritative Flutter loop-control prevention gate'
Assert-C34GPowerShellOwner -Path $launcherPath -Label 'C34G founder launcher'
Assert-C34GPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C34GPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C34G interrupted-postbuild recovery owner'
Assert-C34GPowerShellOwner `
  -Path $cycleOwnerPath `
...
Assert-C34G -Condition (
  ...
  $cycleOwner.IndexOf(
    '$manifestRelative = "$evidenceRelative/source-manifest-c34g-registry-2664.txt"',
    [StringComparison]::Ordinal
  ) -ge 0 -and
  $cycleOwner.IndexOf('C34G source-cycle prerequisite preflight passed: exact state, registry-2664 manifest and focused manifest paths exist; testsStarted=false.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $cycleOwner.IndexOf('registryEntryCount = 2664', [StringComparison]::Ordinal) -ge 0 -and
  $cycleOwner.IndexOf('832FFF89B6D7BD8E990878267252F6B58FBE085337E5990B4F68D3F8859EA2D7', [StringComparison]::Ordinal) -ge 0 -and
...
$gateNeedle = 'scripts/check-uaw-c34g-r60-71-authentication-no-regression-release-readiness.ps1'
...
Assert-C34G -Condition (
  ...
  $launcher.IndexOf('C34G r60.71 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C34G founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C34G binding, gate order or single appbundle owner changed.'
Assert-C34G -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C34GFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C34GFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C34GFile `
...
foreach ($successorGatePath in @(
  ...
  $successorGate = Resolve-C34GFile -Path $successorGatePath `
...
$googleStatePath = Resolve-C34GFile `
...
Assert-C34GSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C34GFile `
...
Assert-C34GSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C34GFile `
...
Write-Output (
  'C34G r60.71 no-regression release gate passed: ' +
