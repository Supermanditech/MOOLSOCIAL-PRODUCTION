...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c34e.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34E postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34ERecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34E existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34ERecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34ERecovery -Condition (
...
function Write-C34ERecoveryState {
  param(
  ...
  Assert-C34ERecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$candidateId =
  'UAW-C34E-R60-69-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRoot =
  'artifacts/quality/uaw-c34e-r60-69-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C34ERecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34ERecoveryPath `
...
$transientGoogleServices = Resolve-C34ERecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34ERecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.69-2026081369-release.aab"
$sealedPath = Resolve-C34ERecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34ERecoveryPath `
...
Assert-C34ERecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34ERecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.69' -and
  [string]$provenance.versionCode -ceq '2026081369' -and
...
$buildLog = Resolve-C34ERecoveryPath `
...
Assert-C34ERecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34ERecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34e-recovery-state-write'
  Write-C34ERecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34e-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c34e-r60-69-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C34E existing-AAB postbuild lifecycle recovery passed: ' +
