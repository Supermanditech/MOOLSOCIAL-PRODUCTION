...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34J postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
$evidenceRoot =
  'artifacts/quality/uaw-c34j-r60-74-release-lifecycle-atomic-parity-preparation-20260817-01'

function Assert-C34JRecovery([bool]$Condition, [string]$Message) {
  if (-not $Condition) {
    throw "C34J existing-AAB recovery rejected: $Message"
  }
}
function Resolve-RecoveryPath([string]$Path, [string]$Label) {
  Assert-C34JRecovery -Condition (
...
$artifactRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.74-2026081374-release.aab"
...
Assert-C34JRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.74' -and
  [string]$provenance.versionCode -ceq '2026081374' -and
...
if ($Mode -ceq 'apply') {
  $transition = Resolve-RecoveryPath `
    -Path 'scripts/invoke-release-lifecycle-transition-c34j.ps1' `
  ...
  & (Join-Path $root `
    'scripts/check-uaw-c34j-r60-74-release-lifecycle-atomic-parity-readiness.ps1') `
    -Phase postbuild -StatePath $StatePath -RepositoryRoot $root
}

Write-Output (
  'C34J existing-AAB postbuild lifecycle recovery passed: ' +
