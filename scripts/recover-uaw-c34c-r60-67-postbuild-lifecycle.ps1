...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c34c.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C34C postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C34CRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34C existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C34CRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C34CRecovery -Condition (
...
function Write-C34CRecoveryState {
  param(
  ...
  Assert-C34CRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$candidateId =
  'UAW-C34C-R60-67-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$evidenceRoot =
  'artifacts/quality/uaw-c34c-r60-67-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C34CRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C34CRecoveryPath `
...
$transientGoogleServices = Resolve-C34CRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C34CRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.67-2026081367-release.aab"
$sealedPath = Resolve-C34CRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C34CRecoveryPath `
...
Assert-C34CRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C34CRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.67' -and
  [string]$provenance.versionCode -ceq '2026081367' -and
...
$buildLog = Resolve-C34CRecoveryPath `
...
Assert-C34CRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C34CRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c34c-recovery-state-write'
  Write-C34CRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c34c-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c34c-r60-67-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C34C existing-AAB postbuild lifecycle recovery passed: ' +
