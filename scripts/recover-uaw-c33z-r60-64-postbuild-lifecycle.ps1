...
param(
  ...
  [string]$StatePath =
    'config/successor-aab-regression-hard-gate-state-c33z.json',
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'C33Z postbuild lifecycle recovery requires PowerShell 7 or newer.'
}
...
function Assert-C33ZRecovery {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C33Z existing-AAB postbuild recovery rejected: $Message"
  }
}

function Resolve-C33ZRecoveryPath {
  param(
    [Parameter(Mandatory)][string]$RelativePath,
    [Parameter(Mandatory)][string]$Label,
    [switch]$AllowMissing
  )
  Assert-C33ZRecovery -Condition (
...
function Write-C33ZRecoveryState {
  param(
  ...
  Assert-C33ZRecovery -Condition (-not (Test-Path -LiteralPath $temporary)) `
...
$evidenceRoot =
  'artifacts/quality/uaw-c33z-r60-64-authentication-no-regression-preparation-20260816-01'
$stateFile = Resolve-C33ZRecoveryPath -RelativePath $StatePath -Label 'state'
$state = Get-Content -Raw -LiteralPath $stateFile | ConvertFrom-Json
$aggregateFile = Resolve-C33ZRecoveryPath `
...
$transientGoogleServices = Resolve-C33ZRecoveryPath `
  -RelativePath ([string]$state.runtimeConfiguration.transientGoogleServicesPath) `
  -Label 'transient Google Services file' `
  -AllowMissing
Assert-C33ZRecovery -Condition (-not (Test-Path -LiteralPath $transientGoogleServices)) `
  -Message 'transient Google Services file remains.'

$sealedRelative =
  "$evidenceRoot/MoolSocial-1.0.0-r60.64-2026081364-release.aab"
$sealedPath = Resolve-C33ZRecoveryPath -RelativePath $sealedRelative -Label 'sealed AAB'
$provenanceCandidates = @()
foreach ($attempt in 1..5) {
  $relative = "$evidenceRoot/06-release-aab-provenance-attempt-$attempt.json"
  $resolved = Resolve-C33ZRecoveryPath `
...
Assert-C33ZRecovery -Condition ($provenanceCandidates.Count -eq 1) `
...
Assert-C33ZRecovery -Condition (
  ...
  [string]$provenance.versionName -ceq '1.0.0-r60.64' -and
  [string]$provenance.versionCode -ceq '2026081364' -and
...
$buildLog = Resolve-C33ZRecoveryPath `
...
Assert-C33ZRecovery -Condition ($credentialHits.Count -eq 0) `
...
if ($Mode -ceq 'apply') {
  ...
  Write-C33ZRecoveryState `
    -State $state `
    -Path $stateFile `
    -Suffix '.c33z-recovery-state-write'
  Write-C33ZRecoveryState `
    -State $aggregate `
    -Path $aggregateFile `
    -Suffix '.c33z-recovery-aggregate-write'
  & (Join-Path $root `
    'scripts/check-uaw-c33z-r60-64-authentication-no-regression-release-readiness.ps1') `
    -Phase postbuild `
    -StatePath $StatePath `
    -RepositoryRoot $root
}
...
Write-Output (
  'C33Z existing-AAB postbuild lifecycle recovery passed: ' +
