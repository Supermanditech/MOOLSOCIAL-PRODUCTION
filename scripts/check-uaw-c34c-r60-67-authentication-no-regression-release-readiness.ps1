...
param(
  ...
  [string]$StatePath = 'config/successor-aab-regression-hard-gate-state-c34c.json',
...
function Assert-C34C {
  param(
    [Parameter(Mandatory)][bool]$Condition,
    [Parameter(Mandatory)][string]$Message
  )
  if (-not $Condition) {
    throw "C34C r60.67 no-regression release gate rejected: $Message"
  }
}

function Resolve-C34CFile {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  Assert-C34C -Condition (-not [string]::IsNullOrWhiteSpace($Path)) `
...
function Assert-C34CSanitizedText {
  param(
  ...
  foreach ($pattern in @(
    ...
    Assert-C34C -Condition (-not [regex]::IsMatch($Text, $pattern)) `
      -Message "$Label contains a credential-, token- or private-key-shaped value."
  }
}

function Assert-C34CPowerShellOwner {
  param(
    [Parameter(Mandatory)][string]$Path,
    [Parameter(Mandatory)][string]$Label
  )
  ...
  Assert-C34C -Condition (@($errors).Count -eq 0) `
    -Message "$Label PowerShell parser rejected the current owner."
}

function Assert-C34CManifestCurrent {
  param([Parameter(Mandatory)][string]$ManifestPath)
  foreach ($line in Get-Content -LiteralPath $ManifestPath) {
    $match = [regex]::Match($line, '^([0-9A-F]{64})  (.+)$')
    Assert-C34C -Condition $match.Success -Message 'source-manifest row is malformed.'
    $owner = Resolve-C34CFile -Path $match.Groups[2].Value -Label 'sealed source owner'
    Assert-C34C -Condition (
...
$ticketId = 'UAW-C34C-R60-67-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE'
$ticketPath = Resolve-C34CFile `
  -Path 'config/uaw-c34c-r60-67-authentication-no-regression-play-oppo-acceptance-ticket.json' `
  -Label 'C34C ticket'
$ticketRaw = Get-Content -Raw -LiteralPath $ticketPath
Assert-C34CSanitizedText -Text $ticketRaw -Label 'C34C ticket'
Assert-C34C -Condition (
...
Assert-C34C -Condition (
  ...
  [string]$ticket.candidate.versionName -ceq '1.0.0-r60.67' -and
  [string]$ticket.candidate.versionCode -ceq '2026081367' -and
  ...
  @($ticket.parentTickets) -contains
    'UAW-C34B-R60-66-AUTHENTICATION-NO-REGRESSION-PLAY-OPPO-ACCEPTANCE' -and
  [string]$ticket.classificationReason -match 'REG2658' -and
...
$uploadRunbookPath = Resolve-C34CFile `
  -Path 'docs/quality/UAW-C34C-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' `
  -Label 'C34C pre-sealed Internal Testing upload runbook'
$uploadRunbook = Get-Content -Raw -LiteralPath $uploadRunbookPath
Assert-C34CSanitizedText -Text $uploadRunbook -Label 'C34C upload runbook'
Assert-C34C -Condition (
...
$resolvedStatePath = Resolve-C34CFile -Path $StatePath -Label 'C34C state'
$stateRaw = Get-Content -Raw -LiteralPath $resolvedStatePath
Assert-C34CSanitizedText -Text $stateRaw -Label 'C34C state'
$state = $stateRaw | ConvertFrom-Json
$aggregatePath = Resolve-C34CFile `
  -Path ([string]$state.aggregateStatePath) `
  -Label 'C34C aggregate'
$aggregateRaw = Get-Content -Raw -LiteralPath $aggregatePath
Assert-C34CSanitizedText -Text $aggregateRaw -Label 'C34C aggregate'
...
Assert-C34C -Condition (
  ...
  [string]$state.candidate.versionName -ceq '1.0.0-r60.67' -and
  [string]$state.candidate.versionCode -ceq '2026081367' -and
...
Assert-C34C -Condition (
  [string]$aggregate.contractId -ceq
    'MOOLSOCIAL-C34C-R60-67-AUTHENTICATION-NO-REGRESSION-RELEASE-AGGREGATE-001' -and
  [string]$aggregate.ticketId -ceq $ticketId -and
  [string]$aggregate.candidate.id -ceq $ticketId -and
  [string]$aggregate.candidate.versionName -ceq '1.0.0-r60.67' -and
  [string]$aggregate.candidate.versionCode -ceq '2026081367'
...
$browserQualificationPath = Resolve-C34CFile `
  -Path ([string]$state.presealUploadWorkflow.evidencePath) `
  -Label 'C34C pre-sealed browser qualification evidence'
$founderAuthorizationPath = Resolve-C34CFile `
  -Path ([string]$state.presealUploadWorkflow.founderAuthorizationPath) `
  -Label 'C34C end-to-end founder authorization'
Assert-C34C -Condition (
  [string]$state.presealUploadWorkflow.runbookPath -ceq
    'docs/quality/UAW-C34C-PRESEALED-INTERNAL-TESTING-UPLOAD-RUNBOOK-20260816.md' -and
  [string]$state.presealUploadWorkflow.evidencePath -ceq
    'docs/quality/UAW-C34C-PRESEALED-INTERNAL-TESTING-BROWSER-QUALIFICATION-20260816.md' -and
  [string]$state.presealUploadWorkflow.founderAuthorizationPath -ceq
    'docs/quality/UAW-C34C-END-TO-END-FOUNDER-AUTHORIZATION-20260816.md' -and
...
Assert-C34C -Condition (
  $historicalContracts.Count -eq 18 -and
  $aggregateHistoricalContracts.Count -eq 18
) -Message 'exact failed/rejected candidate history is incomplete.'

$failedContract = $historicalContracts[0]
$failedStatePath = Resolve-C34CFile -Path ([string]$failedContract.statePath) `
...
foreach ($index in 1..2) {
  ...
  $rejectedStatePath = Resolve-C34CFile -Path ([string]$rejectedContract.statePath) `
...
$rejectedC33NStatePath = Resolve-C34CFile `
...
$rejectedC33OStatePath = Resolve-C34CFile `
...
$rejectedC33PStatePath = Resolve-C34CFile -Path ([string]$rejectedC33PContract.statePath) -Label 'rejected r60.54 state'
...
$rejectedC33QStatePath = Resolve-C34CFile -Path ([string]$rejectedC33QContract.statePath) -Label 'rejected r60.55 state'
...
$rejectedC33RStatePath = Resolve-C34CFile -Path ([string]$rejectedC33RContract.statePath) -Label 'rejected r60.56 state'
...
$rejectedC33SStatePath = Resolve-C34CFile -Path ([string]$rejectedC33SContract.statePath) -Label 'rejected r60.57 state'
...
$rejectedC33TStatePath = Resolve-C34CFile `
...
$rejectedC33UStatePath = Resolve-C34CFile `
...
$rejectedC33VStatePath = Resolve-C34CFile `
...
$rejectedC33WStatePath = Resolve-C34CFile `
...
$rejectedC33XStatePath = Resolve-C34CFile `
...
$rejectedC33YStatePath = Resolve-C34CFile `
...
$rejectedC33ZStatePath = Resolve-C34CFile `
...
$rejectedC34AStatePath = Resolve-C34CFile `
...
$rejectedC34BStatePath = Resolve-C34CFile `
...
$phoneState = Get-Content -Raw -LiteralPath (
  Resolve-C34CFile -Path ([string]$state.sourcePrerequisites.phoneReadinessPath) `
    -Label 'Phone readiness state'
) | ConvertFrom-Json
$emailSourceState = Get-Content -Raw -LiteralPath (
  Resolve-C34CFile -Path ([string]$state.sourcePrerequisites.emailSourceStatePath) `
    -Label 'email source state'
) | ConvertFrom-Json
$emailLiveState = Get-Content -Raw -LiteralPath (
  Resolve-C34CFile -Path ([string]$state.sourcePrerequisites.emailLiveReadinessPath) `
...
$launcherPath = Resolve-C34CFile `
  -Path ([string]$state.releaseBinding.founderLauncher) `
  -Label 'C34C founder launcher'
$wrapperPath = Resolve-C34CFile `
  -Path ([string]$state.releaseBinding.authoritativeAabWrapper) `
  -Label 'generic single-AAB wrapper'
$recoveryPath = Resolve-C34CFile `
  -Path ([string]$state.releaseBinding.postbuildRecoveryOwner) `
  -Label 'C34C interrupted-postbuild recovery owner'
$cycleOwnerPath = Resolve-C34CFile `
  -Path ([string]$state.releaseBinding.sourceCycleOwner) `
  -Label 'C34C predeclared source-cycle owner'
$resultRetentionPath = Resolve-C34CFile `
  -Path ([string]$state.releaseBinding.launcherResultRetentionOwner) `
  -Label 'founder launcher result-retention owner'
Assert-C34CPowerShellOwner -Path $launcherPath -Label 'C34C founder launcher'
Assert-C34CPowerShellOwner -Path $wrapperPath -Label 'generic single-AAB wrapper'
Assert-C34CPowerShellOwner `
  -Path $recoveryPath `
  -Label 'C34C interrupted-postbuild recovery owner'
Assert-C34CPowerShellOwner `
  -Path $cycleOwnerPath `
...
$gateNeedle = 'scripts/check-uaw-c34c-r60-67-authentication-no-regression-release-readiness.ps1'
...
Assert-C34C -Condition (
  ...
  $launcher.IndexOf('C34C r60.67 founder-owned visible console confirmed.', [StringComparison]::Ordinal) -ge 0 -and
  ...
  $launcher.IndexOf(
    "throw 'C34C founder launcher stopped after cleanup; reconcile repository state before any retry.'",
    [StringComparison]::Ordinal
...
) -Message 'generic wrapper C34C binding, gate order or single appbundle owner changed.'
Assert-C34C -Condition (
  $recovery.IndexOf(
...
$scopeGate = Resolve-C34CFile -Path 'scripts/check-mvp-scope-gate-state.ps1' -Label 'MVP scope gate'
if ($Phase -ceq 'source') {
  & $scopeGate `
    -StatePath $ScopePath `
...
$mvpStatePath = Resolve-C34CFile -Path $ScopePath -Label 'MVP state'
...
$flutterClassificationGate = Resolve-C34CFile `
...
$googleStatePath = Resolve-C34CFile `
...
Assert-C34CSanitizedText -Text $googleStateRaw -Label 'Google live-readiness state'
...
foreach ($googleFact in $googleFacts) {
  ...
  $googleEvidencePath = Resolve-C34CFile `
...
Assert-C34CSanitizedText -Text $registryRaw -Label 'regression registry'
...
$focusedManifestPath = Resolve-C34CFile `
...
Write-Output (
  'C34C r60.67 no-regression release gate passed: ' +
