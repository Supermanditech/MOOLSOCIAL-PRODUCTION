[CmdletBinding()]
param([Parameter(Mandatory)][ValidatePattern('^[a-f0-9]{40}$')][string]$ExpectedSourceHead, [Parameter(Mandatory)][ValidatePattern('^[a-f0-9]{40}$')][string]$ApplicationSource)
...
$folder = Join-Path $root 'apps/mobile/build/cursor-review-r66.27'
$apk = Join-Path $folder 'artifacts/uaw-cursor-sku-navigation-20260915-device-review-debug.apk'
$provenance = Join-Path $folder 'artifacts/uaw-cursor-sku-navigation-20260915-build-provenance.txt'
...
$metadataPath=Join-Path $PSScriptRoot 'r66.27-apk-metadata-verified.log'
...
if ($id -cne 'com.moolsocial.app.cursorreview' -or $name -cne '1.0.0-r66.27-cursorreview' -or $code -cne '2026091503') { throw 'Package/version differs from authorized candidate' }
...
$receipt = [ordered]@{
  candidate=$candidate; sourceHead=$ExpectedSourceHead; applicationSource=$ApplicationSource;
  ...
  permittedDevice='TG8HCYTGGQT885OF'; installation='not_yet_performed'; deviceVerification='twelve_SKU_records_pending_device_qualification'
}
$receiptPath=Join-Path $PSScriptRoot 'r66.27-apk-identity.json'
...
[IO.File]::WriteAllLines((Join-Path $PSScriptRoot 'r66.27-apk-signer.log'),$signature,[Text.UTF8Encoding]::new($false))
