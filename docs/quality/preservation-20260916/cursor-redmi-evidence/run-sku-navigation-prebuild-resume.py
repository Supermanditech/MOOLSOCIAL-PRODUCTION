...
checks={}
for name,shell,script,args,marker in jobs[:4]:
 log=e/(name+'.log'); result=e/(name+'.execution.json')
 prior=json.loads(read(result))
 assert prior['passed'] is True and prior['exitCode']==0 and prior['sha256']==sha(log)
 assert marker in read(log)
 checks[log.name]={'sha256':sha(log),'requiredMarker':marker,'exitCode':0}
assert not (e/'navigation-admission-negative.result.json').exists()
old=jobs[4]
jobs[4]=('navigation-admission-negative-02','pwsh',old[2],old[3],old[4])
for name,shell,script,args,marker in jobs[4:]:
 log=e/(name+'.log');result=e/(name+'.execution.json')
