...
root = pathlib.Path(__file__).parent / 'r66.28-captures'
root.mkdir(exist_ok=True)
proof = pathlib.Path(__file__).parent / 'r66.28-installation-receipt.json'
...
assert installed['applicationSource'] == 'f0fc06a92bb43627ec4ca952e8996a888ec96ac2'
assert installed['installedChecksumEqual'] and installed['versionCode'] == 2026091601
label = sys.argv[1]
...
assert b'versionCode=2026091601 ' in identity
assert b'versionName=1.0.0-r66.28-cursorreview' in identity
time.sleep(5)
...
record = {'sourceHead': installed['sourceHead'], 'apkSha256': installed['apkSha256'], 'installationReceiptSha256': receipt_sha, 'fontScale': adb('shell', 'settings', 'get', 'system', 'font_scale').stdout.decode().strip(), 'label': label, 'serial': serial, 'version': '1.0.0-r66.28-cursorreview', 'capturedAt': datetime.datetime.now().astimezone().isoformat(), 'uiautomatorExit': dump.returncode, 'stderrBytes': len(dump.stderr), 'files': {ext: hashlib.sha256(base.with_suffix(ext).read_bytes()).hexdigest() for ext in ['.png', '.xml', '.stderr.txt']}}
...
# Allow Android accessibility-dump teardown before the next injected gesture.
time.sleep(2)
