...
root = pathlib.Path(__file__).parent / 'successor-captures'
...
assert installed['applicationSource'] == '253cbe16da07f069c878bed8f0f5722b8c4aa29c'
assert installed['installedChecksumEqual'] and installed['versionCode'] == 2026091501
label = sys.argv[1]
...
assert b'versionCode=2026091501 ' in identity
assert b'versionName=1.0.0-r66.25-cursorreview' in identity
time.sleep(2)
...
record = {'sourceHead': installed['sourceHead'], 'apkSha256': installed['apkSha256'], 'installationReceiptSha256': receipt_sha, 'fontScale': adb('shell', 'settings', 'get', 'system', 'font_scale').stdout.decode().strip(), 'label': label, 'serial': serial, 'version': '1.0.0-r66.25-cursorreview', 'capturedAt': datetime.datetime.now().astimezone().isoformat(), 'uiautomatorExit': dump.returncode, 'stderrBytes': len(dump.stderr), 'files': {ext: hashlib.sha256(base.with_suffix(ext).read_bytes()).hexdigest() for ext in ['.png', '.xml', '.stderr.txt']}}
