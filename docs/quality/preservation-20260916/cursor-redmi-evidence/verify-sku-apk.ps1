...
$folder = Join-Path $root 'apps/mobile/build/cursor-review-r66.25'
$apk = Join-Path $folder 'artifacts/uaw-cursor-sku-metadata-20260915-device-review-debug.apk'
$provenance = Join-Path $folder 'artifacts/uaw-cursor-sku-metadata-20260915-build-provenance.txt'
...
if ($id -cne 'com.moolsocial.app.cursorreview' -or $name -cne '1.0.0-r66.25-cursorreview' -or $code -cne '2026091501') { throw 'Package/version differs from authorized candidate' }
...
$receipt = [ordered]@{
  ...
  permittedDevice='TG8HCYTGGQT885OF'; installation='not_yet_performed'; deviceVerification='SKU_M01_through_M05_pending_device_qualification'
}
