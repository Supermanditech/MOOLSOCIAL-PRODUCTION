...
root = pathlib.Path(__file__).parent / 'r66.27-captures'
root.mkdir(exist_ok=True)
proof = pathlib.Path(__file__).parent / 'r66.27-installation-receipt.json'
...
assert installed['applicationSource'] == '64ca4d757cffc1cc1fa575b84004cd827e6695ab'
assert installed['installedChecksumEqual'] and installed['versionCode'] == 2026091503
label = sys.argv[1]
...
assert b'versionCode=2026091503 ' in identity
assert b'versionName=1.0.0-r66.27-cursorreview' in identity
time.sleep(2)
...
record = {'sourceHead': installed['sourceHead'], 'apkSha256': installed['apkSha256'], 'installationReceiptSha256': receipt_sha, 'fontScale': adb('shell', 'settings', 'get', 'system', 'font_scale').stdout.decode().strip(), 'label': label, 'serial': serial, 'version': '1.0.0-r66.27-cursorreview', 'capturedAt': datetime.datetime.now().astimezone().isoformat(), 'uiautomatorExit': dump.returncode, 'stderrBytes': len(dump.stderr), 'files': {ext: hashlib.sha256(base.with_suffix(ext).read_bytes()).hexdigest() for ext in ['.png', '.xml', '.stderr.txt']}}
