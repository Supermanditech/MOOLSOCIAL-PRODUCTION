...
before=json.loads(read(e/"sku-rows-media-combined-before-03.json"))
...
runs={};receipts={}
for stage in ["sku-row-media-analysis-06","sku-provider-refresh-06","sku-rows-media-combined-03","sku-rows-media-combined-04"]:
 ...
 if "combined" in stage:
  ...
  assert match and tuple(map(int,match.groups()))==(2453,27),terminal
  r.update(passed=2453,skipped=27,failed=0)
...
print(json.dumps({"qualified":True,"sourceOwners":3044,"eachCombinedPassed":2453,"eachCombinedSkipped":27,"receiptSha256":sha(out)}))
