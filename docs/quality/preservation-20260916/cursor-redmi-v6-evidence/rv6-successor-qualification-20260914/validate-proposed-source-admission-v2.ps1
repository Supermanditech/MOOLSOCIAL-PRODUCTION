...
function git {
  ...
  if ($script:scenario -ceq 'wrong-branch' -and $a -contains '--show-current') { $script:triggered=$true; return 'work/unapproved' }
  if ($script:scenario -ceq 'missing-ancestor' -and $a -contains 'merge-base') { $script:triggered=$true; $global:LASTEXITCODE=1; return }
  if ($script:scenario -ceq 'committed-source-drift' -and $a -contains 'diff' -and $a -contains 'HEAD') { $script:triggered=$true; $global:LASTEXITCODE=1; return }
  if ($script:scenario -ceq 'working-source-drift' -and $a -contains 'diff' -and -not ($a -contains 'HEAD')) { $script:triggered=$true; $global:LASTEXITCODE=1; return }
  if ($script:scenario -ceq 'untracked-source' -and $a -contains 'ls-files') { $script:triggered=$true; return 'apps/mobile/lib/unapproved.dart' }
  if ($script:scenario -ceq 'git-failure') { $script:triggered=$true; $global:LASTEXITCODE=128; return }
  if ($a -contains '--show-current') { return 'work/cursor-ui/redmi-v6-audit-20260913' }
  if ($a -contains 'merge-base' -or $a -contains 'diff' -or $a -contains 'ls-files') { return }
  throw 'Unexpected Git operation in admission test'
}
foreach ($case in @('wrong-branch','missing-ancestor','committed-source-drift','working-source-drift','untracked-source','git-failure')) {
  $script:scenario=$case; $script:triggered=$false
  if (Test-IntegratedStoreBuyReviewSource $pin) { throw "Fail-open: $case" }
  if (-not $script:triggered) { throw "Rejection did not reach intended boundary: $case" }
