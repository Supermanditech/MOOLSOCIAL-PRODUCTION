  ...
  if ($ProductionLane -cne 'integration') {
    ...
    if ($root.Replace('\','/').TrimEnd('/') -ceq 'C:/GUARANTEED OUTCOME/MOOLSOCIAL-WORKTREE-CURSOR-redmi-v6-audit-20260913') {
      ...
      if ($head -ceq $successorParent) {
        ...
        Assert-Coordination ((@($successorDirty | Sort-Object) -join '|') -ceq (@($successorControls | Sort-Object) -join '|')) 'Successor admission must contain exactly four reviewed checker owners.'
      } else {
      ...
      $primaryEvidenceCoordinationOwnerKeys = @($successorControls | ForEach-Object { $_.ToLowerInvariant() })
    }
    # END founder SUCCESSOR BUILD admission 20260914
    if (
      $ProductionLane -ceq 'cursor_ui' -and
