...
for number in range(23, 32):
    ...
    rows.append(['rv622-' + path.stem, 'UAW-CURSOR-REDMI-RV6-LANGUAGE-20260914', data['device'], path.with_suffix('.png').as_posix(), data['pngSha256'], path.with_suffix('.png').stat().st_size, 'physical-device-fixture-setup-reviewed' if number in (24,25,29,30,31) else 'physical-device-fixture-setup'])
...
manifest.write_text(json.dumps({'scope':'D014 fixture setup only; no native-link acceptance', 'files':files}, indent=2)+'\n', encoding='utf-8')
