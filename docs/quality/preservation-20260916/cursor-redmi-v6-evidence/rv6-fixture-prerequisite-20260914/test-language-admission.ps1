...
$start = $checker.IndexOf('      # Founder-approved RV6 fixture plus D005-C01:')
...
$owners = @('apps/mobile/lib/ui_v2/buy/buy_v2_catalogue.dart', 'apps/mobile/test/ui_v2/buy/buy_v2_partner_catalogue_test.dart', 'apps/mobile/lib/main.dart', 'apps/mobile/lib/app/ui_review_language_store.dart', 'apps/mobile/test/app/ui_review_language_store_test.dart')
...
function git {
  ...
  if ($args -contains 'ls-files') { $global:LASTEXITCODE = 0; return }
...
$cases = @(
  ...
  @{name='changed-main'; pass=$false; corrupt=2},
  @{name='changed-store'; pass=$false; corrupt=3},
  @{name='changed-language-test'; pass=$false; corrupt=4},
...
foreach ($case in $cases) {
  ...
  switch ($case.name) {
    'extra-owner' { $script:caseDelta += 'apps/mobile/lib/unrelated.dart' }
